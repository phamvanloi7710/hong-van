# T147 — Ổn định Angular danh sách Product
...
- **Phụ thuộc:** `T146`
- **File queue:** `tasks/T147_STABILIZE_PRODUCT_LIST_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular danh sách Product phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Filters/tabs/pagination/bulk/permission/loading/error.
2. vi/en/zh.
3. responsive.
...
- Hoàn thành đúng trọng tâm: Filters/tabs/pagination/bulk/permission/loading/error.
- Hoàn thành đúng trọng tâm: vi/en/zh.
- Hoàn thành đúng trọng tâm: responsive.
...
1. Cập nhật task `T147` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T147` trong message và chỉ stage file thuộc task.
