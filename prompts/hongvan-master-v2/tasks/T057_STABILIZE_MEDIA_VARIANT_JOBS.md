# T057 — Ổn định job tạo thumbnail WebP AVIF
...
- **Phụ thuộc:** `T056`
- **File queue:** `tasks/T057_STABILIZE_MEDIA_VARIANT_JOBS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định job tạo thumbnail WebP AVIF phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Queue idempotent, retry state, cleanup partial files.
2. metadata dimensions/quality đúng.
...
- Hoàn thành đúng trọng tâm: Queue idempotent, retry state, cleanup partial files.
- Hoàn thành đúng trọng tâm: metadata dimensions/quality đúng.
...
1. Cập nhật task `T057` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T057` trong message và chỉ stage file thuộc task.
