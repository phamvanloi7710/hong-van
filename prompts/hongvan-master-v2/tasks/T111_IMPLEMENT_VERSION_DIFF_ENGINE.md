# T111 — Triển khai diff summary theo block ID
...
- **Phụ thuộc:** `T110`
- **File queue:** `tasks/T111_IMPLEMENT_VERSION_DIFF_ENGINE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai diff summary theo block ID phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Add/remove/move/props/style/visibility/settings/theme changes.
2. payload bounded.
3. không trả unsanitized HTML.
...
- Hoàn thành đúng trọng tâm: Add/remove/move/props/style/visibility/settings/theme changes.
- Hoàn thành đúng trọng tâm: payload bounded.
- Hoàn thành đúng trọng tâm: không trả unsanitized HTML.
...
1. Cập nhật task `T111` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T111` trong message và chỉ stage file thuộc task.
