...
- **Phụ thuộc:** `T207`
- **File queue:** `tasks/T208_SEARCH_FINAL_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng test/E2E Search phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Accent queries/injection/highlight/filters/pagination/related/reindex/privacy/locale/mobile pass.
...
- Hoàn thành đúng trọng tâm: Accent queries/injection/highlight/filters/pagination/related/reindex/privacy/locale/mobile pass.
...
1. Cập nhật task `T208` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T208` trong message và chỉ stage file thuộc task.
