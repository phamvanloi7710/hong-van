...
- **Bao phủ prompt gốc:** `P00,P01`
- **Phụ thuộc:** `T001`
- **File queue:** `tasks/T002_INVENTORY_REPOSITORY_STRUCTURE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Kiểm kê cấu trúc repository và source boundaries phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Lập inventory thư mục/file chính và bounded contexts.
2. phân biệt source chính, output build, binary, generated và source tham chiếu.
3. ghi fingerprint số lượng/kích thước ở mức đủ dùng.
...
- Hoàn thành đúng trọng tâm: Lập inventory thư mục/file chính và bounded contexts.
- Hoàn thành đúng trọng tâm: phân biệt source chính, output build, binary, generated và source tham chiếu.
- Hoàn thành đúng trọng tâm: ghi fingerprint số lượng/kích thước ở mức đủ dùng.
...
1. Cập nhật task `T002` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T002` trong message và chỉ stage file thuộc task.
