# T138 — Làm cứng collision, canonical và redirect


## Metadata

- **Giai đoạn:** H — Menus, Global Regions và Public Routing
- **Bao phủ prompt gốc:** `P31,P43`
- **Phụ thuộc:** `T137`
- **File queue:** `tasks/T138_HARDEN_ROUTE_COLLISIONS_CANONICAL.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng collision, canonical và redirect phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Reserved slug/collision detection.
2. canonical/hreflang.
3. old slug 301 exact.
4. no loop/open redirect.
...
- Hoàn thành đúng trọng tâm: Reserved slug/collision detection.
- Hoàn thành đúng trọng tâm: canonical/hreflang.
- Hoàn thành đúng trọng tâm: old slug 301 exact.
- Hoàn thành đúng trọng tâm: no loop/open redirect.
...
1. Cập nhật task `T138` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T138` trong message và chỉ stage file thuộc task.
