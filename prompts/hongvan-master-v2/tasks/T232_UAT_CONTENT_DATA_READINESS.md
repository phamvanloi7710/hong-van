# T232 — UAT, nội dung và dữ liệu sẵn sàng


## Metadata

- **Giai đoạn:** M — Seeders, QA, CI, Deployment, Operations, Security, UAT và Handover
- **Bao phủ prompt gốc:** `P54`
- **Phụ thuộc:** `T231`
- **File queue:** `tasks/T232_UAT_CONTENT_DATA_READINESS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

UAT, nội dung và dữ liệu sẵn sàng phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `prompts/hongvan-master-v2/state/`
...
```powershell
rg -n "UAT|placeholder|demo|example\.com|TODO|content\ readiness" BackEnd Admin docs scripts docker 2>$null
```
...
1. Scenario matrix all roles/locales/devices.
2. company content/settings/media/SEO/forms.
3. data import rehearsal.
4. owner sign-off.
...
- Mọi scenario có expected result, actual result và bằng chứng.
- Dữ liệu demo không bị nhầm thành dữ liệu doanh nghiệp thật.
- Owner gate được ghi rõ APPROVED, REJECTED hoặc BLOCKED, không tự suy diễn.
- Hoàn thành đúng trọng tâm: Scenario matrix all roles/locales/devices.
- Hoàn thành đúng trọng tâm: company content/settings/media/SEO/forms.
- Hoàn thành đúng trọng tâm: data import rehearsal.
- Hoàn thành đúng trọng tâm: owner sign-off.
...
```text
Chạy UAT script/checklist trên hongvan.local hoặc môi trường staging thật.
git diff --check
```
...
1. Cập nhật task `T232` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T232` trong message và chỉ stage file thuộc task.
