# T130 — Làm cứng nesting, link và localization Menu


## Metadata

- **Giai đoạn:** H — Menus, Global Regions và Public Routing
- **Bao phủ prompt gốc:** `P30`
- **Phụ thuộc:** `T129`
- **File queue:** `tasks/T130_HARDEN_MENU_NESTING_LOCALIZATION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng nesting, link và localization Menu phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Chặn cycle/orphan/depth.
2. internal route refs vs safe external URL.
3. locale fallback và active state.
...
- Hoàn thành đúng trọng tâm: Chặn cycle/orphan/depth.
- Hoàn thành đúng trọng tâm: internal route refs vs safe external URL.
- Hoàn thành đúng trọng tâm: locale fallback và active state.
...
1. Cập nhật task `T130` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T130` trong message và chỉ stage file thuộc task.
