# T144 — Làm cứng pricing modes và resolver


## Metadata

- **Giai đoạn:** I — Product Catalog hoàn chỉnh
- **Bao phủ prompt gốc:** `P32`
- **Phụ thuộc:** `T143`
- **File queue:** `tasks/T144_HARDEN_PRODUCT_PRICING.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng pricing modes và resolver phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. fixed/from/range/contact/market/dealer/quantity validation.
2. currency/unit/note/visibility.
3. zero/null fallback.
...
- Hoàn thành đúng trọng tâm: fixed/from/range/contact/market/dealer/quantity validation.
- Hoàn thành đúng trọng tâm: currency/unit/note/visibility.
- Hoàn thành đúng trọng tâm: zero/null fallback.
...
1. Cập nhật task `T144` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T144` trong message và chỉ stage file thuộc task.
