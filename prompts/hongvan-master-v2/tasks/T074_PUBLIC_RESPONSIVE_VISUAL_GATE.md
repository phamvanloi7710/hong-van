# T074 — Cổng responsive và visual public foundation


## Metadata

- **Giai đoạn:** E — Public Blade foundation và Theme Studio
- **Bao phủ prompt gốc:** `P19,P46`
- **Phụ thuộc:** `T073`
- **File queue:** `tasks/T074_PUBLIC_RESPONSIVE_VISUAL_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng responsive và visual public foundation phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. 1440/768/390 không overflow.
2. keyboard/focus.
3. ảnh/font/icon không 404.
4. visual evidence.
...
- Hoàn thành đúng trọng tâm: 1440/768/390 không overflow.
- Hoàn thành đúng trọng tâm: keyboard/focus.
- Hoàn thành đúng trọng tâm: ảnh/font/icon không 404.
- Hoàn thành đúng trọng tâm: visual evidence.
...
1. Cập nhật task `T074` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T074` trong message và chỉ stage file thuộc task.
