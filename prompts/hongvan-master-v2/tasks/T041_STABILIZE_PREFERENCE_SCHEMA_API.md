# T041 — Ổn định schema và API user preferences
...
- **Phụ thuộc:** `T040`
- **File queue:** `tasks/T041_STABILIZE_PREFERENCE_SCHEMA_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định schema và API user preferences phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Namespace/key/value allowlist.
2. serial writes chống lost update.
3. delete/reset có default rõ.
...
- Hoàn thành đúng trọng tâm: Namespace/key/value allowlist.
- Hoàn thành đúng trọng tâm: serial writes chống lost update.
- Hoàn thành đúng trọng tâm: delete/reset có default rõ.
...
1. Cập nhật task `T041` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T041` trong message và chỉ stage file thuộc task.
