...
- **Phụ thuộc:** `T036`
- **File queue:** `tasks/T037_STABILIZE_ROLE_MANAGEMENT.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định quản lý role phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD role, gán permission, system role protection và transaction.
2. không xóa role còn ràng buộc trái policy.
...
- Hoàn thành đúng trọng tâm: CRUD role, gán permission, system role protection và transaction.
- Hoàn thành đúng trọng tâm: không xóa role còn ràng buộc trái policy.
...
1. Cập nhật task `T037` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T037` trong message và chỉ stage file thuộc task.
