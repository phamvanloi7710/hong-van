# T123 — Ổn định UI schedule và timezone
...
- **Phụ thuộc:** `T122`
- **File queue:** `tasks/T123_STABILIZE_SCHEDULE_UI_TIMEZONE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định UI schedule và timezone phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Typed date/time.
2. Asia/Ho_Chi_Minh + UTC preview.
3. past validation.
4. cancel/reschedule/error 409/422.
...
- Hoàn thành đúng trọng tâm: Typed date/time.
- Hoàn thành đúng trọng tâm: Asia/Ho_Chi_Minh + UTC preview.
- Hoàn thành đúng trọng tâm: past validation.
- Hoàn thành đúng trọng tâm: cancel/reschedule/error 409/422.
...
1. Cập nhật task `T123` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T123` trong message và chỉ stage file thuộc task.
