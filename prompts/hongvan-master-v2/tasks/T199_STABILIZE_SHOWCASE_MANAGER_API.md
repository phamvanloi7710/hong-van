# T199 — Ổn định manager và API Showcase
...
- **Phụ thuộc:** `T198`
- **File queue:** `tasks/T199_STABILIZE_SHOWCASE_MANAGER_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định manager và API Showcase phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD/publish/archive/trash/restore/order/featured.
2. policy/audit/cache/media usage.
...
- Hoàn thành đúng trọng tâm: CRUD/publish/archive/trash/restore/order/featured.
- Hoàn thành đúng trọng tâm: policy/audit/cache/media usage.
...
1. Cập nhật task `T199` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T199` trong message và chỉ stage file thuộc task.
