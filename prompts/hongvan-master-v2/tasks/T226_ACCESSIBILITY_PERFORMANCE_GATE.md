# T226 — Cổng accessibility và performance


## Metadata

- **Giai đoạn:** M — Seeders, QA, CI, Deployment, Operations, Security, UAT và Handover
- **Bao phủ prompt gốc:** `P46`
- **Phụ thuộc:** `T225`
- **File queue:** `tasks/T226_ACCESSIBILITY_PERFORMANCE_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng accessibility và performance phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Keyboard/focus/labels/contrast/landmarks.
2. mobile.
3. asset/query/bundle budgets.
4. no arbitrary threshold inflation.
...
- Hoàn thành đúng trọng tâm: Keyboard/focus/labels/contrast/landmarks.
- Hoàn thành đúng trọng tâm: mobile.
- Hoàn thành đúng trọng tâm: asset/query/bundle budgets.
- Hoàn thành đúng trọng tâm: no arbitrary threshold inflation.
...
1. Cập nhật task `T226` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T226` trong message và chỉ stage file thuộc task.
