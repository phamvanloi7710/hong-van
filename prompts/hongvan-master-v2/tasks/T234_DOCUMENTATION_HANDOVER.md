# T234 — Tài liệu và bàn giao cuối


## Metadata

- **Giai đoạn:** M — Seeders, QA, CI, Deployment, Operations, Security, UAT và Handover
- **Bao phủ prompt gốc:** `P56`
- **Phụ thuộc:** `T233`
- **File queue:** `tasks/T234_DOCUMENTATION_HANDOVER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tài liệu và bàn giao cuối phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
```powershell
rg -n "pending|deprecated|old|P29|P30|P31|production" BackEnd Admin docs scripts docker 2>$null
```
...
1. Install/dev/test/build/deploy/backup/restore/operations/security/admin user guides/architecture/ADR.
...
- Tài liệu phản ánh implementation thật, không giữ trạng thái DONE cũ sai lệch.
- Runbook đủ cho dev mới cài, test, build, deploy và rollback.
- Không chứa credential, token hoặc dữ liệu cá nhân thật.
- Hoàn thành đúng trọng tâm: Install/dev/test/build/deploy/backup/restore/operations/security/admin user guides/architecture/ADR.
- Hoàn thành đúng trọng tâm: handover checklist.
...
```text
git diff --check
Kiểm tra mọi command/path trong tài liệu tồn tại hoặc được đánh dấu ví dụ.
```
...
1. Cập nhật task `T234` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T234` trong message và chỉ stage file thuộc task.
