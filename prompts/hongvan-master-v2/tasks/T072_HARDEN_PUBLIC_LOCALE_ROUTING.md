# T072 — Làm cứng route locale public


## Metadata

- **Giai đoạn:** E — Public Blade foundation và Theme Studio
- **Bao phủ prompt gốc:** `P14,P18`
- **Phụ thuộc:** `T071`
- **File queue:** `tasks/T072_HARDEN_PUBLIC_LOCALE_ROUTING.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng route locale public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. vi không prefix, en/zh prefix.
2. disabled locale fallback.
3. canonical redirect không loop.
...
- Hoàn thành đúng trọng tâm: vi không prefix, en/zh prefix.
- Hoàn thành đúng trọng tâm: disabled locale fallback.
- Hoàn thành đúng trọng tâm: canonical redirect không loop.
...
1. Cập nhật task `T072` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T072` trong message và chỉ stage file thuộc task.
