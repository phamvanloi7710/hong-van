# T187 — Ổn định assignment, status và notes
...
- **Phụ thuộc:** `T186`
- **File queue:** `tasks/T187_STABILIZE_LEAD_WORKFLOW.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định assignment, status và notes phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Transition policy/visibility/history append-only.
2. assigned/team scopes.
3. follow-up dates UTC.
...
- Hoàn thành đúng trọng tâm: Transition policy/visibility/history append-only.
- Hoàn thành đúng trọng tâm: assigned/team scopes.
- Hoàn thành đúng trọng tâm: follow-up dates UTC.
...
1. Cập nhật task `T187` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T187` trong message và chỉ stage file thuộc task.
