# T235 — Rà soát lại toàn bộ source sau triển khai
...
- **Phụ thuộc:** `T234`
- **File queue:** `tasks/T235_FULL_SOURCE_REAUDIT.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà soát lại toàn bộ source sau triển khai phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Reconcile source/routes/schema/UI/tests/docs/commits.
2. không dựa vào status cũ.
3. ghi findings có severity/evidence/owner.
...
- Hoàn thành đúng trọng tâm: Reconcile source/routes/schema/UI/tests/docs/commits.
- Hoàn thành đúng trọng tâm: không dựa vào status cũ.
- Hoàn thành đúng trọng tâm: ghi findings có severity/evidence/owner.
...
1. Cập nhật task `T235` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T235` trong message và chỉ stage file thuộc task.
