...
- **Phụ thuộc:** `T155`
- **File queue:** `tasks/T156_PRODUCT_FINAL_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng test/E2E Product phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Pricing permutations, CRUD/policy, public catalog/detail/quote/SEO/PB blocks/vi-en-zh/mobile pass.
...
- Hoàn thành đúng trọng tâm: Pricing permutations, CRUD/policy, public catalog/detail/quote/SEO/PB blocks/vi-en-zh/mobile pass.
...
1. Cập nhật task `T156` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T156` trong message và chỉ stage file thuộc task.
