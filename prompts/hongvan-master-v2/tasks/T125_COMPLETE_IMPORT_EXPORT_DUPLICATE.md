# T125 — Hoàn thiện import, export và duplicate P29


## Metadata

- **Giai đoạn:** G — Ổn định P28 và P29
- **Bao phủ prompt gốc:** `P29`
- **Phụ thuộc:** `T124`
- **File queue:** `tasks/T125_COMPLETE_IMPORT_EXPORT_DUPLICATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Hoàn thiện import, export và duplicate P29 phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Manifest/schema/block versions/media public IDs.
2. validate/dry-run/mapping/execute.
3. duplicate fresh block IDs.
4. round-trip.
...
- Hoàn thành đúng trọng tâm: Manifest/schema/block versions/media public IDs.
- Hoàn thành đúng trọng tâm: validate/dry-run/mapping/execute.
- Hoàn thành đúng trọng tâm: duplicate fresh block IDs.
- Hoàn thành đúng trọng tâm: round-trip.
...
1. Cập nhật task `T125` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T125` trong message và chỉ stage file thuộc task.
