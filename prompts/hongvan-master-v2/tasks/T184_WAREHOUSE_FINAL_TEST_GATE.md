...
- **Phụ thuộc:** `T183`
- **File queue:** `tasks/T184_WAREHOUSE_FINAL_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng test/E2E Warehouses phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Admin/public/map/privacy/request/lead/PB/SEO/locale/mobile tests pass.
...
- Hoàn thành đúng trọng tâm: Admin/public/map/privacy/request/lead/PB/SEO/locale/mobile tests pass.
...
1. Cập nhật task `T184` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T184` trong message và chỉ stage file thuộc task.
