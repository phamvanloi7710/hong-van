# T233 — Production cutover có kiểm soát


> **External gate:** Không được mark DONE nếu chưa có production thật, quyền deploy, backup, TLS/domain và owner approval.

## Metadata

- **Giai đoạn:** M — Seeders, QA, CI, Deployment, Operations, Security, UAT và Handover
- **Bao phủ prompt gốc:** `P55`
- **Phụ thuộc:** `T232`
- **File queue:** `tasks/T233_PRODUCTION_CUTOVER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Production cutover có kiểm soát phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Preflight backup/TLS/domain/secrets/migrations/build/queue.
2. deploy/smoke/rollback.
3. chỉ DONE khi môi trường thật và owner approval.
...
- Hoàn thành đúng trọng tâm: Preflight backup/TLS/domain/secrets/migrations/build/queue.
- Hoàn thành đúng trọng tâm: deploy/smoke/rollback.
- Hoàn thành đúng trọng tâm: chỉ DONE khi môi trường thật và owner approval.
...
1. Cập nhật task `T233` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T233` trong message và chỉ stage file thuộc task.
