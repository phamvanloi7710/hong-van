# T047 — Ổn định chi nhánh và giờ làm việc
...
- **Phụ thuộc:** `T046`
- **File queue:** `tasks/T047_STABILIZE_BRANCHES_BUSINESS_HOURS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định chi nhánh và giờ làm việc phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD/order/status.
2. global và per-branch hours.
3. timezone/overnight/closed day validation.
...
- Hoàn thành đúng trọng tâm: CRUD/order/status.
- Hoàn thành đúng trọng tâm: global và per-branch hours.
- Hoàn thành đúng trọng tâm: timezone/overnight/closed day validation.
...
1. Cập nhật task `T047` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T047` trong message và chỉ stage file thuộc task.
