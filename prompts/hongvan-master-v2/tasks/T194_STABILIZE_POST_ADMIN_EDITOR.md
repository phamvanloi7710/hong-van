# T194 — Ổn định Angular CMS editor
...
- **Phụ thuộc:** `T193`
- **File queue:** `tasks/T194_STABILIZE_POST_ADMIN_EDITOR.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular CMS editor phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Three tabs/rich editor/media/preview/schedule/taxonomy.
2. typed/i18n/permission/dirty guard.
...
- Hoàn thành đúng trọng tâm: Three tabs/rich editor/media/preview/schedule/taxonomy.
- Hoàn thành đúng trọng tâm: typed/i18n/permission/dirty guard.
...
1. Cập nhật task `T194` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T194` trong message và chỉ stage file thuộc task.
