# T193 — Ổn định manager, schedule và slug history


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P39`
- **Phụ thuộc:** `T192`
- **File queue:** `tasks/T193_STABILIZE_POST_MANAGER_SCHEDULE_SLUGS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định manager, schedule và slug history phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD/publish/schedule/archive/trash.
2. old slug redirect contract.
3. cache/audit/idempotency.
...
- Hoàn thành đúng trọng tâm: CRUD/publish/schedule/archive/trash.
- Hoàn thành đúng trọng tâm: old slug redirect contract.
- Hoàn thành đúng trọng tâm: cache/audit/idempotency.
...
1. Cập nhật task `T193` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T193` trong message và chỉ stage file thuộc task.
