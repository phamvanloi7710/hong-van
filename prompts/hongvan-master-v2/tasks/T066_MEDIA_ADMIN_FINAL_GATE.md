# T066 — Cổng cuối Media và Admin shell


## Metadata

- **Giai đoạn:** D — Admin shell và Media Manager
- **Bao phủ prompt gốc:** `P06,P16,P17`
- **Phụ thuộc:** `T065`
- **File queue:** `tasks/T066_MEDIA_ADMIN_FINAL_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng cuối Media và Admin shell phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Backend tests, Angular tests/build, Playwright media, mobile/desktop và security checks đều pass.
2. không console error.
...
- Hoàn thành đúng trọng tâm: Backend tests, Angular tests/build, Playwright media, mobile/desktop và security checks đều pass.
- Hoàn thành đúng trọng tâm: không console error.
...
1. Cập nhật task `T066` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T066` trong message và chỉ stage file thuộc task.
