# T089 — Làm cứng renderer và class resolver layout


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P22`
- **Phụ thuộc:** `T088`
- **File queue:** `tasks/T089_HARDEN_LAYOUT_RENDERERS_CLASSES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng renderer và class resolver layout phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Semantic markup.
2. responsive token allowlist.
3. không raw grid/CSS selector/style injection.
...
- Hoàn thành đúng trọng tâm: Semantic markup.
- Hoàn thành đúng trọng tâm: responsive token allowlist.
- Hoàn thành đúng trọng tâm: không raw grid/CSS selector/style injection.
...
1. Cập nhật task `T089` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T089` trong message và chỉ stage file thuộc task.
