# T060 — Làm cứng Media usage registry
...
- **Phụ thuộc:** `T059`
- **File queue:** `tasks/T060_HARDEN_MEDIA_USAGE_TRACKING.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng Media usage registry phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Owner type/id contract.
2. sync stale references.
3. không orphan usage và không xóa media đang dùng.
...
- Hoàn thành đúng trọng tâm: Owner type/id contract.
- Hoàn thành đúng trọng tâm: sync stale references.
- Hoàn thành đúng trọng tâm: không orphan usage và không xóa media đang dùng.
...
1. Cập nhật task `T060` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T060` trong message và chỉ stage file thuộc task.
