# T004 — Đóng dấu và bảo vệ source tham chiếu chỉ đọc


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P01,P03`
- **Phụ thuộc:** `T003`
- **File queue:** `tasks/T004_FINGERPRINT_READONLY_SOURCES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Đóng dấu và bảo vệ source tham chiếu chỉ đọc phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Kiểm kê Template, FrontEndTemplate và SourceIntegrations.
2. ghi hash/fingerprint tái lập.
3. thêm kiểm tra phát hiện thay đổi ngoài ý muốn mà không commit tài sản có giấy phép.
...
- Hoàn thành đúng trọng tâm: Kiểm kê Template, FrontEndTemplate và SourceIntegrations.
- Hoàn thành đúng trọng tâm: ghi hash/fingerprint tái lập.
- Hoàn thành đúng trọng tâm: thêm kiểm tra phát hiện thay đổi ngoài ý muốn mà không commit tài sản có giấy phép.
...
1. Cập nhật task `T004` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T004` trong message và chỉ stage file thuộc task.
