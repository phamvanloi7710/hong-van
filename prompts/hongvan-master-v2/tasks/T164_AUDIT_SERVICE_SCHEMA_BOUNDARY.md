# T164 — Rà schema Service và boundary chuyên ngành
...
- **Phụ thuộc:** `T163`
- **File queue:** `tasks/T164_AUDIT_SERVICE_SCHEMA_BOUNDARY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema Service và boundary chuyên ngành phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Categories/services/translations/media/CTA.
2. không duplicate transport/warehouse detail ownership.
...
- Hoàn thành đúng trọng tâm: Categories/services/translations/media/CTA.
- Hoàn thành đúng trọng tâm: không duplicate transport/warehouse detail ownership.
...
1. Cập nhật task `T164` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T164` trong message và chỉ stage file thuộc task.
