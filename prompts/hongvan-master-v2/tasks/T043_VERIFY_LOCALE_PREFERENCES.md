# T043 — Xác minh locale Admin theo user


## Metadata

- **Giai đoạn:** C — Authentication, RBAC, Preferences, Settings, Localization và Audit
- **Bao phủ prompt gốc:** `P12,P14`
- **Phụ thuộc:** `T042`
- **File queue:** `tasks/T043_VERIFY_LOCALE_PREFERENCES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Xác minh locale Admin theo user phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Locale đổi ngay, lưu server, gửi X-Locale.
2. catalog vi/en/zh đầy đủ và fallback xác định.
...
- Hoàn thành đúng trọng tâm: Locale đổi ngay, lưu server, gửi X-Locale.
- Hoàn thành đúng trọng tâm: catalog vi/en/zh đầy đủ và fallback xác định.
...
1. Cập nhật task `T043` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T043` trong message và chỉ stage file thuộc task.
