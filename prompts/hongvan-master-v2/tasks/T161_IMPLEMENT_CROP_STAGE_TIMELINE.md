# T161 — Hoàn thiện timeline giai đoạn cây trồng


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P34`
- **Phụ thuộc:** `T160`
- **File queue:** `tasks/T161_IMPLEMENT_CROP_STAGE_TIMELINE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Hoàn thiện timeline giai đoạn cây trồng phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Ordered stages/content/media/product recommendations.
2. semantic/a11y.
3. no fabricated agronomy claims.
...
- Hoàn thành đúng trọng tâm: Ordered stages/content/media/product recommendations.
- Hoàn thành đúng trọng tâm: semantic/a11y.
- Hoàn thành đúng trọng tâm: no fabricated agronomy claims.
...
1. Cập nhật task `T161` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T161` trong message và chỉ stage file thuộc task.
