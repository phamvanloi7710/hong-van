# T179 — Ổn định Angular Warehouses
...
- **Phụ thuộc:** `T178`
- **File queue:** `tasks/T179_STABILIZE_WAREHOUSE_ADMIN_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular Warehouses phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Tabs/dialogs/media/facility/service/hours/map settings.
2. typed forms/i18n/mobile.
...
- Hoàn thành đúng trọng tâm: Tabs/dialogs/media/facility/service/hours/map settings.
- Hoàn thành đúng trọng tâm: typed forms/i18n/mobile.
...
1. Cập nhật task `T179` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T179` trong message và chỉ stage file thuộc task.
