# T133 — Triển khai Angular editor Global Regions
...
- **Phụ thuộc:** `T132`
- **File queue:** `tasks/T133_IMPLEMENT_GLOBAL_REGION_EDITOR.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai Angular editor Global Regions phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Reuse editor components an toàn.
2. region selector/history/preview.
3. không duplicate renderer client.
...
- Hoàn thành đúng trọng tâm: Reuse editor components an toàn.
- Hoàn thành đúng trọng tâm: region selector/history/preview.
- Hoàn thành đúng trọng tâm: không duplicate renderer client.
...
1. Cập nhật task `T133` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T133` trong message và chỉ stage file thuộc task.
