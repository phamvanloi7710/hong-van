# T212 — Làm cứng sitemap index, shards và cache


## Metadata

- **Giai đoạn:** L — SEO, Analytics và Dashboard
- **Bao phủ prompt gốc:** `P43`
- **Phụ thuộc:** `T211`
- **File queue:** `tasks/T212_HARDEN_SITEMAP_GENERATOR.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng sitemap index, shards và cache phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. All published localized entities.
2. lastmod/canonical/exclusions.
3. size/count/shard/cache invalidation.
...
- Hoàn thành đúng trọng tâm: All published localized entities.
- Hoàn thành đúng trọng tâm: lastmod/canonical/exclusions.
- Hoàn thành đúng trọng tâm: size/count/shard/cache invalidation.
...
1. Cập nhật task `T212` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T212` trong message và chỉ stage file thuộc task.
