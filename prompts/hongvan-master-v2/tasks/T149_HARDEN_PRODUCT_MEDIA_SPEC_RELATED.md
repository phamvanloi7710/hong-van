# T149 — Làm cứng media, specifications và related products
...
- **Phụ thuộc:** `T148`
- **File queue:** `tasks/T149_HARDEN_PRODUCT_MEDIA_SPEC_RELATED.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng media, specifications và related products phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Media usage/order/primary.
2. typed specs.
3. related constraints/no self/duplicates/N+1.
...
- Hoàn thành đúng trọng tâm: Media usage/order/primary.
- Hoàn thành đúng trọng tâm: typed specs.
- Hoàn thành đúng trọng tâm: related constraints/no self/duplicates/N+1.
...
1. Cập nhật task `T149` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T149` trong message và chỉ stage file thuộc task.
