# T197 — Cổng test/E2E News Content
...
- **Phụ thuộc:** `T196`
- **File queue:** `tasks/T197_POST_FINAL_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng test/E2E News Content phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Sanitizer/schedule/slugs/Admin/public/RSS/PB/SEO/locale/mobile tests pass.
...
- Hoàn thành đúng trọng tâm: Sanitizer/schedule/slugs/Admin/public/RSS/PB/SEO/locale/mobile tests pass.
...
1. Cập nhật task `T197` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T197` trong message và chỉ stage file thuộc task.
