# T165 — Ổn định manager và API Service


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P35`
- **Phụ thuộc:** `T164`
- **File queue:** `tasks/T165_STABILIZE_SERVICE_MANAGER_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định manager và API Service phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD/publish/archive/trash/restore.
2. CTA source/policy/audit/cache/query.
...
- Hoàn thành đúng trọng tâm: CRUD/publish/archive/trash/restore.
- Hoàn thành đúng trọng tâm: CTA source/policy/audit/cache/query.
...
1. Cập nhật task `T165` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T165` trong message và chỉ stage file thuộc task.
