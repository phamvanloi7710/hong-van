# T011 — Chuẩn hóa danh mục lệnh test, lint, build và E2E


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P04-P07,P48-P50`
- **Phụ thuộc:** `T010`
- **File queue:** `tasks/T011_INVENTORY_TEST_AND_BUILD_COMMANDS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Chuẩn hóa danh mục lệnh test, lint, build và E2E phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Đối chiếu composer scripts, npm scripts, artisan commands và Playwright.
2. loại tài liệu lệnh không tồn tại.
3. ghi ma trận fast gate và full gate.
...
- Hoàn thành đúng trọng tâm: Đối chiếu composer scripts, npm scripts, artisan commands và Playwright.
- Hoàn thành đúng trọng tâm: loại tài liệu lệnh không tồn tại.
- Hoàn thành đúng trọng tâm: ghi ma trận fast gate và full gate.
...
1. Cập nhật task `T011` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T011` trong message và chỉ stage file thuộc task.
