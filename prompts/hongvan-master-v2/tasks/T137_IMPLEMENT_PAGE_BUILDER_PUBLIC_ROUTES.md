# T137 — Triển khai route public động cho Page Builder


## Metadata

- **Giai đoạn:** H — Menus, Global Regions và Public Routing
- **Bao phủ prompt gốc:** `P31`
- **Phụ thuộc:** `T136`
- **File queue:** `tasks/T137_IMPLEMENT_PAGE_BUILDER_PUBLIC_ROUTES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai route public động cho Page Builder phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Resolve page published version/theme/regions/data sources.
2. cache key contract.
3. 404 draft/unpublished.
...
- Hoàn thành đúng trọng tâm: Resolve page published version/theme/regions/data sources.
- Hoàn thành đúng trọng tâm: cache key contract.
- Hoàn thành đúng trọng tâm: 404 draft/unpublished.
...
1. Cập nhật task `T137` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T137` trong message và chỉ stage file thuộc task.
