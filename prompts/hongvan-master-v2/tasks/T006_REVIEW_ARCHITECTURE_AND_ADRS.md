# T006 — Rà kiến trúc tổng thể và ADR


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P02`
- **Phụ thuộc:** `T005`
- **File queue:** `tasks/T006_REVIEW_ARCHITECTURE_AND_ADRS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà kiến trúc tổng thể và ADR phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Kiểm tra monorepo, bounded contexts và luồng public/admin.
2. phát hiện ADR trùng/xung đột/lỗi thời.
3. cập nhật chỉ mục quyết định và tạo ADR mới chỉ khi contract thay đổi.
...
- Hoàn thành đúng trọng tâm: Kiểm tra monorepo, bounded contexts và luồng public/admin.
- Hoàn thành đúng trọng tâm: phát hiện ADR trùng/xung đột/lỗi thời.
- Hoàn thành đúng trọng tâm: cập nhật chỉ mục quyết định và tạo ADR mới chỉ khi contract thay đổi.
...
1. Cập nhật task `T006` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T006` trong message và chỉ stage file thuộc task.
