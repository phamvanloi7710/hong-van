# T025 — Rà toàn bộ rate limiter


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P09,P10,P15`
- **Phụ thuộc:** `T024`
- **File queue:** `tasks/T025_REVIEW_RATE_LIMITERS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà toàn bộ rate limiter phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Phân tách auth/public forms/search/uploads/preview/API.
2. key theo user/IP phù hợp.
3. production nhiều instance dùng shared store.
...
- Hoàn thành đúng trọng tâm: Phân tách auth/public forms/search/uploads/preview/API.
- Hoàn thành đúng trọng tâm: key theo user/IP phù hợp.
- Hoàn thành đúng trọng tâm: production nhiều instance dùng shared store.
...
1. Cập nhật task `T025` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T025` trong message và chỉ stage file thuộc task.
