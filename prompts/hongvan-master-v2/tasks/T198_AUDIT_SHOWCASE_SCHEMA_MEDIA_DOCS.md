# T198 — Rà schema Showcase, Media và documents


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P40`
- **Phụ thuộc:** `T197`
- **File queue:** `tasks/T198_AUDIT_SHOWCASE_SCHEMA_MEDIA_DOCS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema Showcase, Media và documents phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Galleries/items/partners/certifications/projects/translations/media.
2. visibility/comments/FK.
...
- Hoàn thành đúng trọng tâm: Galleries/items/partners/certifications/projects/translations/media.
- Hoàn thành đúng trọng tâm: visibility/comments/FK.
...
1. Cập nhật task `T198` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T198` trong message và chỉ stage file thuộc task.
