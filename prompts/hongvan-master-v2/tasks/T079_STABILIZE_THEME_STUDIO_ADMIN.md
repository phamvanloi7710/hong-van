# T079 — Ổn định Angular Theme Studio
...
- **Phụ thuộc:** `T078`
- **File queue:** `tasks/T079_STABILIZE_THEME_STUDIO_ADMIN.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular Theme Studio phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Typed controls, history, preview, publish/rollback dialogs.
2. permission/i18n/build sync.
...
- Hoàn thành đúng trọng tâm: Typed controls, history, preview, publish/rollback dialogs.
- Hoàn thành đúng trọng tâm: permission/i18n/build sync.
...
1. Cập nhật task `T079` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T079` trong message và chỉ stage file thuộc task.
