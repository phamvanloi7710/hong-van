# T183 — Đăng ký Warehouse blocks, SEO và cache


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P24,P37,P42`
- **Phụ thuộc:** `T182`
- **File queue:** `tasks/T183_REGISTER_WAREHOUSE_BLOCKS_SEO.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Đăng ký Warehouse blocks, SEO và cache phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Grid/detail/facility/form blocks.
2. meta/schema/breadcrumb/dependency invalidation.
...
- Hoàn thành đúng trọng tâm: Grid/detail/facility/form blocks.
- Hoàn thành đúng trọng tâm: meta/schema/breadcrumb/dependency invalidation.
...
1. Cập nhật task `T183` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T183` trong message và chỉ stage file thuộc task.
