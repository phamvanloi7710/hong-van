# T068 — Chuẩn hóa component và design token public


## Metadata

- **Giai đoạn:** E — Public Blade foundation và Theme Studio
- **Bao phủ prompt gốc:** `P18`
- **Phụ thuộc:** `T067`
- **File queue:** `tasks/T068_STANDARDIZE_PUBLIC_COMPONENTS_TOKENS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Chuẩn hóa component và design token public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/resources/css/public/`
- `BackEnd/resources/js/public/`
...
```powershell
rg -n "layouts\.public|canonical|hreflang|@vite|public\.locale|noindex" BackEnd Admin docs scripts docker 2>$null
```
...
1. Button/link/image/form/heading/container/breadcrumb/alert.
2. focus/responsive/accessibility.
3. không raw style tùy ý.
...
- Public SSR dùng Blade và không gọi API vòng lại chính ứng dụng.
- FrontendTemplate chỉ đọc và mọi asset production đi qua Vite/hash.
- Không đưa cart/checkout/payment/account/wishlist vào hệ thống.
- Hoàn thành đúng trọng tâm: Button/link/image/form/heading/container/breadcrumb/alert.
- Hoàn thành đúng trọng tâm: focus/responsive/accessibility.
- Hoàn thành đúng trọng tâm: không raw style tùy ý.
...
1. Cập nhật task `T068` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T068` trong message và chỉ stage file thuộc task.
