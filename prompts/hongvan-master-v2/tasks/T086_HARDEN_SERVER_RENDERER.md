# T086 — Làm cứng server PageDocumentRenderer


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P21,P22`
- **Phụ thuộc:** `T085`
- **File queue:** `tasks/T086_HARDEN_SERVER_RENDERER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng server PageDocumentRenderer phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Resolve renderer từ registry code.
2. recursive validation.
3. escape props.
4. không arbitrary Blade/view.
...
- Hoàn thành đúng trọng tâm: Resolve renderer từ registry code.
- Hoàn thành đúng trọng tâm: recursive validation.
- Hoàn thành đúng trọng tâm: escape props.
- Hoàn thành đúng trọng tâm: không arbitrary Blade/view.
...
1. Cập nhật task `T086` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T086` trong message và chỉ stage file thuộc task.
