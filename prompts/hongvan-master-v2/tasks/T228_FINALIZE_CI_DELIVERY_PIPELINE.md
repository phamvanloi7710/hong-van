# T228 — Hoàn thiện CI và delivery pipeline


## Metadata

- **Giai đoạn:** M — Seeders, QA, CI, Deployment, Operations, Security, UAT và Handover
- **Bao phủ prompt gốc:** `P50`
- **Phụ thuộc:** `T227`
- **File queue:** `tasks/T228_FINALIZE_CI_DELIVERY_PIPELINE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Hoàn thiện CI và delivery pipeline phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Lockfile reproducible jobs.
2. DB/Redis/services.
3. backend/admin/E2E/security/artifacts/checksums.
4. branch main gates.
...
- Hoàn thành đúng trọng tâm: Lockfile reproducible jobs.
- Hoàn thành đúng trọng tâm: DB/Redis/services.
- Hoàn thành đúng trọng tâm: backend/admin/E2E/security/artifacts/checksums.
- Hoàn thành đúng trọng tâm: branch main gates.
...
1. Cập nhật task `T228` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T228` trong message và chỉ stage file thuộc task.
