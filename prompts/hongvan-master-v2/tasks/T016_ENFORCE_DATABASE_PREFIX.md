# T016 — Cưỡng chế tiền tố hongvan_ cho mọi bảng
...
- **Phụ thuộc:** `T015`
- **File queue:** `tasks/T016_ENFORCE_DATABASE_PREFIX.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cưỡng chế tiền tố hongvan_ cho mọi bảng phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Quét migrations/models/config package tables.
2. chặn bảng framework hoặc pivot không prefix.
3. thêm test CI chống tái phạm.
...
- Hoàn thành đúng trọng tâm: Quét migrations/models/config package tables.
- Hoàn thành đúng trọng tâm: chặn bảng framework hoặc pivot không prefix.
- Hoàn thành đúng trọng tâm: thêm test CI chống tái phạm.
...
1. Cập nhật task `T016` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T016` trong message và chỉ stage file thuộc task.
