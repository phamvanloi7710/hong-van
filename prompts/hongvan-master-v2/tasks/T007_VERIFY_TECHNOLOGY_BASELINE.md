# T007 — Xác minh baseline PHP Laravel Angular Node MySQL Redis


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P02,P04,P05`
- **Phụ thuộc:** `T006`
- **File queue:** `tasks/T007_VERIFY_TECHNOLOGY_BASELINE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Xác minh baseline PHP Laravel Angular Node MySQL Redis phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Đọc composer/package/lockfile và runtime local.
2. không tự nâng major.
3. ghi patch version thật và incompatibility nếu có.
...
- Hoàn thành đúng trọng tâm: Đọc composer/package/lockfile và runtime local.
- Hoàn thành đúng trọng tâm: không tự nâng major.
- Hoàn thành đúng trọng tâm: ghi patch version thật và incompatibility nếu có.
...
1. Cập nhật task `T007` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T007` trong message và chỉ stage file thuộc task.
