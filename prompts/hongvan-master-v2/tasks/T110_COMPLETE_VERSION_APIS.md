# T110 — Hoàn thiện API và Resource lịch sử version
...
- **Phụ thuộc:** `T109`
- **File queue:** `tasks/T110_COMPLETE_VERSION_APIS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Hoàn thiện API và Resource lịch sử version phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. List/show pagination/sort.
2. live/scheduled markers.
3. document chỉ trả khi permission/endpoint cần.
4. public IDs.
...
- Hoàn thành đúng trọng tâm: List/show pagination/sort.
- Hoàn thành đúng trọng tâm: live/scheduled markers.
- Hoàn thành đúng trọng tâm: document chỉ trả khi permission/endpoint cần.
- Hoàn thành đúng trọng tâm: public IDs.
...
1. Cập nhật task `T110` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T110` trong message và chỉ stage file thuộc task.
