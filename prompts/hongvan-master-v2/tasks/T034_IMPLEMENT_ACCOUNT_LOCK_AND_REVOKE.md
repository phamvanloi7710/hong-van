# T034 — Hoàn thiện khóa tài khoản và thu hồi phiên


## Metadata

- **Giai đoạn:** C — Authentication, RBAC, Preferences, Settings, Localization và Audit
- **Bao phủ prompt gốc:** `P10,P11`
- **Phụ thuộc:** `T033`
- **File queue:** `tasks/T034_IMPLEMENT_ACCOUNT_LOCK_AND_REVOKE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Hoàn thiện khóa tài khoản và thu hồi phiên phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
```powershell
rg -n "sanctum|csrf|login|logout|password|session|locked" BackEnd Admin docs scripts docker 2>$null
```
...
1. Lock/activate/reset sessions có permission/audit.
2. session/token bị thu hồi nguyên tử.
3. Admin phản ánh trạng thái đúng.
...
- Auth cùng origin dùng cookie/session và CSRF; không lưu bearer token trong localStorage.
- Session được regenerate sau login và revoke đúng khi logout/khóa tài khoản.
- Thông báo người dùng có đủ vi/en/zh.
- Hoàn thành đúng trọng tâm: Lock/activate/reset sessions có permission/audit.
- Hoàn thành đúng trọng tâm: session/token bị thu hồi nguyên tử.
- Hoàn thành đúng trọng tâm: Admin phản ánh trạng thái đúng.
...
1. Cập nhật task `T034` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T034` trong message và chỉ stage file thuộc task.
