# T132 — Triển khai API và lifecycle Global Regions
...
- **Phụ thuộc:** `T131`
- **File queue:** `tasks/T132_IMPLEMENT_GLOBAL_REGION_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai API và lifecycle Global Regions phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD/draft/preview/publish/rollback.
2. reuse PageDocument validator/renderer.
3. permissions/cache.
...
- Hoàn thành đúng trọng tâm: CRUD/draft/preview/publish/rollback.
- Hoàn thành đúng trọng tâm: reuse PageDocument validator/renderer.
- Hoàn thành đúng trọng tâm: permissions/cache.
...
1. Cập nhật task `T132` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T132` trong message và chỉ stage file thuộc task.
