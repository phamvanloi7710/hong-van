# T121 — Triển khai UI giải quyết conflict
...
- **Phụ thuộc:** `T120`
- **File queue:** `tasks/T121_IMPLEMENT_CONFLICT_RESOLUTION_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai UI giải quyết conflict phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Pause autosave.
2. giữ local document.
3. server/local metadata.
4. load server/save copy/discard options.
5. không force overwrite mặc định.
...
- Hoàn thành đúng trọng tâm: Pause autosave.
- Hoàn thành đúng trọng tâm: giữ local document.
- Hoàn thành đúng trọng tâm: server/local metadata.
- Hoàn thành đúng trọng tâm: load server/save copy/discard options.
- Hoàn thành đúng trọng tâm: không force overwrite mặc định.
...
1. Cập nhật task `T121` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T121` trong message và chỉ stage file thuộc task.
