# T182 — Làm cứng request kho và Lead integration


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P25,P37,P38`
- **Phụ thuộc:** `T181`
- **File queue:** `tasks/T182_HARDEN_WAREHOUSE_REQUEST_LEADS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng request kho và Lead integration phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Goods/area-volume/duration/start/storage/location/contact.
2. consent/dedup/mapping/notifications.
...
- Hoàn thành đúng trọng tâm: Goods/area-volume/duration/start/storage/location/contact.
- Hoàn thành đúng trọng tâm: consent/dedup/mapping/notifications.
...
1. Cập nhật task `T182` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T182` trong message và chỉ stage file thuộc task.
