# T027 — Xác minh queue, cache và Redis contract


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P04,P15`
- **Phụ thuộc:** `T026`
- **File queue:** `tasks/T027_VERIFY_QUEUE_CACHE_REDIS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Xác minh queue, cache và Redis contract phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Kiểm tra cache prefix/tags, queue tables/jobs và Redis fallback local.
2. không để task nặng chạy sync production ngoài ý muốn.
...
- Hoàn thành đúng trọng tâm: Kiểm tra cache prefix/tags, queue tables/jobs và Redis fallback local.
- Hoàn thành đúng trọng tâm: không để task nặng chạy sync production ngoài ý muốn.
...
1. Cập nhật task `T027` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T027` trong message và chỉ stage file thuộc task.
