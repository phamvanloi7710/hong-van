# T188 — Làm cứng notification và follow-up Leads


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P38,P45`
- **Phụ thuộc:** `T187`
- **File queue:** `tasks/T188_HARDEN_LEAD_NOTIFICATIONS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng notification và follow-up Leads phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/Leads/`
- `BackEnd/app/Models/Lead*.php`
- `BackEnd/app/Http/Controllers/Api/V1/Leads/`
- `Admin/src/app/features/leads/`
...
```powershell
rg -n "LeadIntake|idempotency|dedup|assignment|status|retention|export" BackEnd Admin docs scripts docker 2>$null
```
...
1. Queued email/database notification.
2. deep link authorized.
3. retry/idempotency.
4. no sensitive body leak.
...
- Submission gốc bất biến/encrypted theo contract và workflow history append-only.
- Visibility/export tuân thủ permission và ngăn CSV formula injection.
- Retention/anonymization không phá audit hoặc báo cáo được phép.
- Hoàn thành đúng trọng tâm: Queued email/database notification.
- Hoàn thành đúng trọng tâm: deep link authorized.
- Hoàn thành đúng trọng tâm: retry/idempotency.
- Hoàn thành đúng trọng tâm: no sensitive body leak.
...
1. Cập nhật task `T188` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T188` trong message và chỉ stage file thuộc task.
