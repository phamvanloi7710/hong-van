# T024 — Làm cứng filter, sort, search và pagination allowlist
...
- **Phụ thuộc:** `T023`
- **File queue:** `tasks/T024_HARDEN_QUERY_ALLOWLIST.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng filter, sort, search và pagination allowlist phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Không truyền raw column/operator từ request.
2. chuẩn hóa 422 cho key sai.
3. thêm test injection và giới hạn per_page/bulk.
...
- Hoàn thành đúng trọng tâm: Không truyền raw column/operator từ request.
- Hoàn thành đúng trọng tâm: chuẩn hóa 422 cho key sai.
- Hoàn thành đúng trọng tâm: thêm test injection và giới hạn per_page/bulk.
...
1. Cập nhật task `T024` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T024` trong message và chỉ stage file thuộc task.
