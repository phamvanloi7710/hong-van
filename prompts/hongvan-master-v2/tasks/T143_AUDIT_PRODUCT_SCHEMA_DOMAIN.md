# T143 — Rà schema và domain Product
...
- **Phụ thuộc:** `T142`
- **File queue:** `tasks/T143_AUDIT_PRODUCT_SCHEMA_DOMAIN.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema và domain Product phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. 13 catalog tables/relations/comments/soft delete.
2. status/publish lifecycle.
3. no ecommerce tables.
...
- Hoàn thành đúng trọng tâm: 13 catalog tables/relations/comments/soft delete.
- Hoàn thành đúng trọng tâm: status/publish lifecycle.
- Hoàn thành đúng trọng tâm: no ecommerce tables.
...
1. Cập nhật task `T143` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T143` trong message và chỉ stage file thuộc task.
