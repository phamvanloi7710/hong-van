# T077 — Làm cứng CSS compiler và runtime Theme
...
- **Phụ thuộc:** `T076`
- **File queue:** `tasks/T077_HARDEN_THEME_CSS_COMPILER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng CSS compiler và runtime Theme phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Deterministic output, escaping, size limits.
2. preview/published dùng cùng compiler.
...
- Hoàn thành đúng trọng tâm: Deterministic output, escaping, size limits.
- Hoàn thành đúng trọng tâm: preview/published dùng cùng compiler.
...
1. Cập nhật task `T077` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T077` trong message và chỉ stage file thuộc task.
