# T103 — Ổn định inspector và responsive settings


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P26`
- **Phụ thuộc:** `T102`
- **File queue:** `tasks/T103_STABILIZE_INSPECTOR_RESPONSIVE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định inspector và responsive settings phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Typed controls from schema.
2. desktop/tablet/mobile visibility/style.
3. unknown field không render/edit.
...
- Hoàn thành đúng trọng tâm: Typed controls from schema.
- Hoàn thành đúng trọng tâm: desktop/tablet/mobile visibility/style.
- Hoàn thành đúng trọng tâm: unknown field không render/edit.
...
1. Cập nhật task `T103` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T103` trong message và chỉ stage file thuộc task.
