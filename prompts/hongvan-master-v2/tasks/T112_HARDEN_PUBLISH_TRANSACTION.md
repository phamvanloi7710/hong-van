# T112 — Làm cứng transaction Publish now
...
- **Phụ thuộc:** `T111`
- **File queue:** `tasks/T112_HARDEN_PUBLISH_TRANSACTION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng transaction Publish now phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Lock page.
2. concurrency/permission/lock checks.
3. immutable published revision.
4. pointer update và supersede schedule nguyên tử.
...
- Hoàn thành đúng trọng tâm: Lock page.
- Hoàn thành đúng trọng tâm: concurrency/permission/lock checks.
- Hoàn thành đúng trọng tâm: immutable published revision.
- Hoàn thành đúng trọng tâm: pointer update và supersede schedule nguyên tử.
...
1. Cập nhật task `T112` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T112` trong message và chỉ stage file thuộc task.
