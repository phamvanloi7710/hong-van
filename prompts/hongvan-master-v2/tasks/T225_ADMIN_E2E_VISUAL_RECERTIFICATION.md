# T225 — Tái chứng nhận Angular, E2E và Visual QA


## Metadata

- **Giai đoạn:** M — Seeders, QA, CI, Deployment, Operations, Security, UAT và Handover
- **Bao phủ prompt gốc:** `P49`
- **Phụ thuộc:** `T224`
- **File queue:** `tasks/T225_ADMIN_E2E_VISUAL_RECERTIFICATION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái chứng nhận Angular, E2E và Visual QA phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Full lint/unit/build sync/Playwright.
2. critical workflows/public/admin.
3. visual snapshots intentional.
4. console clean.
...
- Hoàn thành đúng trọng tâm: Full lint/unit/build sync/Playwright.
- Hoàn thành đúng trọng tâm: critical workflows/public/admin.
- Hoàn thành đúng trọng tâm: visual snapshots intentional.
- Hoàn thành đúng trọng tâm: console clean.
...
1. Cập nhật task `T225` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T225` trong message và chỉ stage file thuộc task.
