# T045 — Làm cứng registry cấu hình công ty
...
- **Phụ thuộc:** `T044`
- **File queue:** `tasks/T045_HARDEN_COMPANY_SETTING_REGISTRY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng registry cấu hình công ty phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Typed groups/keys/default/validation.
2. cache invalidation theo key.
3. unknown setting bị từ chối.
...
- Hoàn thành đúng trọng tâm: Typed groups/keys/default/validation.
- Hoàn thành đúng trọng tâm: cache invalidation theo key.
- Hoàn thành đúng trọng tâm: unknown setting bị từ chối.
...
1. Cập nhật task `T045` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T045` trong message và chỉ stage file thuộc task.
