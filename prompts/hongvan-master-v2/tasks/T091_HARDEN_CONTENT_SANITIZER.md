# T091 — Làm cứng sanitizer content/media blocks


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P23,P15`
- **Phụ thuộc:** `T090`
- **File queue:** `tasks/T091_HARDEN_CONTENT_SANITIZER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng sanitizer content/media blocks phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. XSS/event/unsafe URL/data URI.
2. one H1.
3. target blank rel.
4. rich text limits.
...
- Hoàn thành đúng trọng tâm: XSS/event/unsafe URL/data URI.
- Hoàn thành đúng trọng tâm: one H1.
- Hoàn thành đúng trọng tâm: target blank rel.
- Hoàn thành đúng trọng tâm: rich text limits.
...
1. Cập nhật task `T091` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T091` trong message và chỉ stage file thuộc task.
