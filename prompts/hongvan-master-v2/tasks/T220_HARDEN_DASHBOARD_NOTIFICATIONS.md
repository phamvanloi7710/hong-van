# T220 — Làm cứng notification Dashboard
...
- **Phụ thuộc:** `T219`
- **File queue:** `tasks/T220_HARDEN_DASHBOARD_NOTIFICATIONS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng notification Dashboard phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Read/read-all ownership.
2. deep links authorized.
3. counts/cache/retry.
4. safe payload.
...
- Hoàn thành đúng trọng tâm: Read/read-all ownership.
- Hoàn thành đúng trọng tâm: deep links authorized.
- Hoàn thành đúng trọng tâm: counts/cache/retry.
- Hoàn thành đúng trọng tâm: safe payload.
...
1. Cập nhật task `T220` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T220` trong message và chỉ stage file thuộc task.
