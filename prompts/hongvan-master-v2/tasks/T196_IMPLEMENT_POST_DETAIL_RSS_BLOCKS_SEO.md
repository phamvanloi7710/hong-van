# T196 — Triển khai detail, related, RSS, blocks và SEO Posts


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P39,P42,P43`
- **Phụ thuộc:** `T195`
- **File queue:** `tasks/T196_IMPLEMENT_POST_DETAIL_RSS_BLOCKS_SEO.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai detail, related, RSS, blocks và SEO Posts phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/Posts/`
- `BackEnd/app/Models/Post*.php`
- `BackEnd/app/Http/Controllers/Api/V1/Posts/`
- `Admin/src/app/features/posts/`
...
```powershell
rg -n "RichTextSanitizer|publish|schedule|slug\ history|RSS|related" BackEnd Admin docs scripts docker 2>$null
```
...
1. Detail/author/date/media.
2. related.
3. RSS valid.
4. post list block.
5. Article schema/canonical.
...
- HTML được sanitize cả lúc lưu và render; không XSS/event handler/unsafe URL.
- Schedule và slug history không gây duplicate content.
- RSS/related chỉ dùng nội dung published theo locale.
- Hoàn thành đúng trọng tâm: Detail/author/date/media.
- Hoàn thành đúng trọng tâm: related.
- Hoàn thành đúng trọng tâm: RSS valid.
- Hoàn thành đúng trọng tâm: post list block.
- Hoàn thành đúng trọng tâm: Article schema/canonical.
...
1. Cập nhật task `T196` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T196` trong message và chỉ stage file thuộc task.
