# T073 — Làm cứng Vite asset và cache headers


## Metadata

- **Giai đoạn:** E — Public Blade foundation và Theme Studio
- **Bao phủ prompt gốc:** `P18,P19`
- **Phụ thuộc:** `T072`
- **File queue:** `tasks/T073_HARDEN_PUBLIC_ASSET_PIPELINE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng Vite asset và cache headers phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Hashed manifest, MIME đúng, immutable headers.
2. không source map production nếu không cần.
...
- Hoàn thành đúng trọng tâm: Hashed manifest, MIME đúng, immutable headers.
- Hoàn thành đúng trọng tâm: không source map production nếu không cần.
...
1. Cập nhật task `T073` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T073` trong message và chỉ stage file thuộc task.
