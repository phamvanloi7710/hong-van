# T064 — Ổn định Media Picker và image editor
...
- **Phụ thuộc:** `T063`
- **File queue:** `tasks/T064_STABILIZE_MEDIA_PICKER_EDITOR.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Media Picker và image editor phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Selection contract, folder/search/filter.
2. crop/rotate metadata và output queue.
3. dialog typed vi/en/zh.
...
- Hoàn thành đúng trọng tâm: Selection contract, folder/search/filter.
- Hoàn thành đúng trọng tâm: crop/rotate metadata và output queue.
- Hoàn thành đúng trọng tâm: dialog typed vi/en/zh.
...
1. Cập nhật task `T064` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T064` trong message và chỉ stage file thuộc task.
