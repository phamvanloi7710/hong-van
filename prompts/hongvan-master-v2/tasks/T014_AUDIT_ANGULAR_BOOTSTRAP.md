# T014 — Rà soát Angular bootstrap, standalone và strict mode


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P05`
- **Phụ thuộc:** `T013`
- **File queue:** `tasks/T014_AUDIT_ANGULAR_BOOTSTRAP.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà soát Angular bootstrap, standalone và strict mode phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Kiểm tra app.config, app.routes, tsconfig strict và providers.
2. không dùng NgModule cũ nếu không cần.
3. đảm bảo environment typed.
...
- Hoàn thành đúng trọng tâm: Kiểm tra app.config, app.routes, tsconfig strict và providers.
- Hoàn thành đúng trọng tâm: không dùng NgModule cũ nếu không cần.
- Hoàn thành đúng trọng tâm: đảm bảo environment typed.
...
1. Cập nhật task `T014` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T014` trong message và chỉ stage file thuộc task.
