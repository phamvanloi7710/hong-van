# T115 — Làm cứng scheduler chống race và chạy trùng
...
- **Phụ thuộc:** `T114`
- **File queue:** `tasks/T115_HARDEN_SCHEDULER_IDEMPOTENCY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng scheduler chống race và chạy trùng phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Claim/lock/processing/completed/failed states.
2. retry/stuck recovery.
3. two-run idempotency.
...
- Hoàn thành đúng trọng tâm: Claim/lock/processing/completed/failed states.
- Hoàn thành đúng trọng tâm: retry/stuck recovery.
- Hoàn thành đúng trọng tâm: two-run idempotency.
...
1. Cập nhật task `T115` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T115` trong message và chỉ stage file thuộc task.
