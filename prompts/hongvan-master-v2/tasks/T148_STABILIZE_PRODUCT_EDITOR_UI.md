# T148 — Ổn định Angular editor Product
...
- **Phụ thuộc:** `T147`
- **File queue:** `tasks/T148_STABILIZE_PRODUCT_EDITOR_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular editor Product phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Typed form translations/pricing/status/SEO/media/specs.
2. validation maps backend.
3. dirty guard.
...
- Hoàn thành đúng trọng tâm: Typed form translations/pricing/status/SEO/media/specs.
- Hoàn thành đúng trọng tâm: validation maps backend.
- Hoàn thành đúng trọng tâm: dirty guard.
...
1. Cập nhật task `T148` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T148` trong message và chỉ stage file thuộc task.
