# T209 — Làm cứng entity registry và meta resolver SEO


## Metadata

- **Giai đoạn:** L — SEO, Analytics và Dashboard
- **Bao phủ prompt gốc:** `P42`
- **Phụ thuộc:** `T208`
- **File queue:** `tasks/T209_HARDEN_SEO_ENTITY_META_RESOLVER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng entity registry và meta resolver SEO phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. All final route types.
2. typed fields/fallback/robots/media variants.
3. no draft/private metadata.
...
- Hoàn thành đúng trọng tâm: All final route types.
- Hoàn thành đúng trọng tâm: typed fields/fallback/robots/media variants.
- Hoàn thành đúng trọng tâm: no draft/private metadata.
...
1. Cập nhật task `T209` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T209` trong message và chỉ stage file thuộc task.
