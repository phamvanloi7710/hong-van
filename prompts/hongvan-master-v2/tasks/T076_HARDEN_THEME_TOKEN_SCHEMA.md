# T076 — Làm cứng schema token Theme
...
- **Phụ thuộc:** `T075`
- **File queue:** `tasks/T076_HARDEN_THEME_TOKEN_SCHEMA.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng schema token Theme phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Allowlist color/font/spacing/radius/shadow/animation.
2. reject CSS/JS/url nguy hiểm.
...
- Hoàn thành đúng trọng tâm: Allowlist color/font/spacing/radius/shadow/animation.
- Hoàn thành đúng trọng tâm: reject CSS/JS/url nguy hiểm.
...
1. Cập nhật task `T076` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T076` trong message và chỉ stage file thuộc task.
