# T126 — Làm cứng edit lock và cổng regression P28-P29
...
- **Phụ thuộc:** `T125`
- **File queue:** `tasks/T126_HARDEN_EDIT_LOCKS_AND_REGRESSION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng edit lock và cổng regression P28-P29 phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Acquire/heartbeat/release/expiry/reconnect/force unlock.
2. lock token save/publish.
3. full P28-P29 backend/Angular/E2E pass.
...
- Hoàn thành đúng trọng tâm: Acquire/heartbeat/release/expiry/reconnect/force unlock.
- Hoàn thành đúng trọng tâm: lock token save/publish.
- Hoàn thành đúng trọng tâm: full P28-P29 backend/Angular/E2E pass.
...
1. Cập nhật task `T126` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T126` trong message và chỉ stage file thuộc task.
