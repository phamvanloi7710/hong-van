# T030 — Cổng tích hợp nền tảng


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P04-P09`
- **Phụ thuộc:** `T029`
- **File queue:** `tasks/T030_FOUNDATION_INTEGRATION_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng tích hợp nền tảng phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Chạy test/build/API smoke/database guards.
2. không mở Identity nếu foundation đỏ.
3. ghi exact commands và kết quả.
...
- Hoàn thành đúng trọng tâm: Chạy test/build/API smoke/database guards.
- Hoàn thành đúng trọng tâm: không mở Identity nếu foundation đỏ.
- Hoàn thành đúng trọng tâm: ghi exact commands và kết quả.
...
1. Cập nhật task `T030` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T030` trong message và chỉ stage file thuộc task.
