# T021 — Làm cứng exception renderer API
...
- **Phụ thuộc:** `T020`
- **File queue:** `tasks/T021_HARDEN_API_EXCEPTION_RENDERING.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng exception renderer API phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Ánh xạ 401/403/404/409/419/422/429/500.
2. không trả class/file/line/trace.
3. giữ thông báo localized an toàn.
...
- Hoàn thành đúng trọng tâm: Ánh xạ 401/403/404/409/419/422/429/500.
- Hoàn thành đúng trọng tâm: không trả class/file/line/trace.
- Hoàn thành đúng trọng tâm: giữ thông báo localized an toàn.
...
1. Cập nhật task `T021` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T021` trong message và chỉ stage file thuộc task.
