...
- **Phụ thuộc:** `T214`
- **File queue:** `tasks/T215_SEO_CRAWL_FINAL_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng crawl và test SEO phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Crawl vi/en/zh final routes.
2. canonical/hreflang/sitemap/robots/redirect/schema/no broken links/duplicate titles.
...
- Hoàn thành đúng trọng tâm: Crawl vi/en/zh final routes.
- Hoàn thành đúng trọng tâm: canonical/hreflang/sitemap/robots/redirect/schema/no broken links/duplicate titles.
...
1. Cập nhật task `T215` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T215` trong message và chỉ stage file thuộc task.
