# T116 — Hoàn thiện cancel và reschedule
...
- **Phụ thuộc:** `T115`
- **File queue:** `tasks/T116_IMPLEMENT_CANCEL_RESCHEDULE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Hoàn thiện cancel và reschedule phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Transaction/audit/idempotency.
2. past/completed schedule rules.
3. UI reflects target revision.
...
- Hoàn thành đúng trọng tâm: Transaction/audit/idempotency.
- Hoàn thành đúng trọng tâm: past/completed schedule rules.
- Hoàn thành đúng trọng tâm: UI reflects target revision.
...
1. Cập nhật task `T116` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T116` trong message và chỉ stage file thuộc task.
