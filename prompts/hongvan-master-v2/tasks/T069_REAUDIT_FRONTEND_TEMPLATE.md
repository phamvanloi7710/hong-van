# T069 — Kiểm kê lại FrontEndTemplate và giấy phép


## Metadata

- **Giai đoạn:** E — Public Blade foundation và Theme Studio
- **Bao phủ prompt gốc:** `P19`
- **Phụ thuộc:** `T068`
- **File queue:** `tasks/T069_REAUDIT_FRONTEND_TEMPLATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Kiểm kê lại FrontEndTemplate và giấy phép phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
2. phân loại plugin/assets/license.
3. loại WooCommerce/e-commerce runtime khỏi port.
...
- Hoàn thành đúng trọng tâm: Fingerprint source.
- Hoàn thành đúng trọng tâm: phân loại plugin/assets/license.
- Hoàn thành đúng trọng tâm: loại WooCommerce/e-commerce runtime khỏi port.
...
1. Cập nhật task `T069` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T069` trong message và chỉ stage file thuộc task.
