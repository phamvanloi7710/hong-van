# T213 — Làm cứng robots, canonical và hreflang


## Metadata

- **Giai đoạn:** L — SEO, Analytics và Dashboard
- **Bao phủ prompt gốc:** `P42,P43`
- **Phụ thuộc:** `T212`
- **File queue:** `tasks/T213_HARDEN_ROBOTS_CANONICAL_HREFLANG.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng robots, canonical và hreflang phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/Seo/`
- `BackEnd/app/Http/Controllers/Seo/`
- `BackEnd/app/Http/Controllers/Api/V1/Seo/`
- `Admin/src/app/features/seo/`
...
```powershell
rg -n "canonical|hreflang|sitemap|robots|redirect|StructuredData|og:" BackEnd Admin docs scripts docker 2>$null
```
...
1. Environment-safe robots.
2. preview/admin/api disallow/noindex.
3. self/alternate/default locale links correct.
...
- Canonical/hreflang/sitemap chỉ trỏ nội dung public hợp lệ.
- Redirect không cho open redirect hoặc chain/loop không kiểm soát.
- Structured data chỉ dùng dữ liệu thật, không bịa Offer/rating/address.
- Hoàn thành đúng trọng tâm: Environment-safe robots.
- Hoàn thành đúng trọng tâm: preview/admin/api disallow/noindex.
- Hoàn thành đúng trọng tâm: self/alternate/default locale links correct.
...
```text
cd BackEnd; php artisan test --filter=Seo
cd BackEnd; php artisan test --filter=Sitemap
cd Admin; npm test -- --run seo
...
1. Cập nhật task `T213` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T213` trong message và chỉ stage file thuộc task.
