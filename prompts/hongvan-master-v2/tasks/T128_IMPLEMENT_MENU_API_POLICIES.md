# T128 — Triển khai API, validation và Policy Menu
...
- **Phụ thuộc:** `T127`
- **File queue:** `tasks/T128_IMPLEMENT_MENU_API_POLICIES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai API, validation và Policy Menu phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD/tree reorder/publish.
2. permission pages/menu namespace.
3. cycle/depth/url allowlist.
...
- Hoàn thành đúng trọng tâm: CRUD/tree reorder/publish.
- Hoàn thành đúng trọng tâm: permission pages/menu namespace.
- Hoàn thành đúng trọng tâm: cycle/depth/url allowlist.
...
1. Cập nhật task `T128` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T128` trong message và chỉ stage file thuộc task.
