# T033 — Làm cứng quên và đặt lại mật khẩu
...
- **Phụ thuộc:** `T032`
- **File queue:** `tasks/T033_HARDEN_FORGOT_RESET_PASSWORD.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng quên và đặt lại mật khẩu phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Không user enumeration.
2. token một lần/hết hạn.
3. rate limit và revoke session theo policy.
...
- Hoàn thành đúng trọng tâm: Không user enumeration.
- Hoàn thành đúng trọng tâm: token một lần/hết hạn.
- Hoàn thành đúng trọng tâm: rate limit và revoke session theo policy.
...
1. Cập nhật task `T033` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T033` trong message và chỉ stage file thuộc task.
