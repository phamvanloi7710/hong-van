# T098 — Làm cứng Blade form và tích hợp Lead


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P25,P38`
- **Phụ thuộc:** `T097`
- **File queue:** `tasks/T098_HARDEN_FORM_RENDER_LEAD_INTEGRATION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng Blade form và tích hợp Lead phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CSRF/honeypot/consent/rate/idempotency.
2. accessible error/success.
...
- Hoàn thành đúng trọng tâm: CSRF/honeypot/consent/rate/idempotency.
- Hoàn thành đúng trọng tâm: accessible error/success.
- Hoàn thành đúng trọng tâm: queue notifications.
...
1. Cập nhật task `T098` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T098` trong message và chỉ stage file thuộc task.
