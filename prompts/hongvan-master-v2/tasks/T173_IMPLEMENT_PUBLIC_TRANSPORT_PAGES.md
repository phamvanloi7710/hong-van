# T173 — Triển khai public capability pages Transportation


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P36,P31`
- **Phụ thuộc:** `T172`
- **File queue:** `tasks/T173_IMPLEMENT_PUBLIC_TRANSPORT_PAGES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai public capability pages Transportation phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Fleet/routes/areas/capabilities published views.
2. privacy-safe details.
3. canonical/cache.
...
- Hoàn thành đúng trọng tâm: Fleet/routes/areas/capabilities published views.
- Hoàn thành đúng trọng tâm: privacy-safe details.
- Hoàn thành đúng trọng tâm: canonical/cache.
...
1. Cập nhật task `T173` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T173` trong message và chỉ stage file thuộc task.
