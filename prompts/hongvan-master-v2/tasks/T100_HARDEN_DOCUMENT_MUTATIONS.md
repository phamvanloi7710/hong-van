# T100 — Làm cứng thao tác document immutable
...
- **Phụ thuộc:** `T099`
- **File queue:** `tasks/T100_HARDEN_DOCUMENT_MUTATIONS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng thao tác document immutable phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Add/move/reorder/nest/duplicate/delete.
2. fresh IDs.
3. parent/depth/count/cycle guards.
...
- Hoàn thành đúng trọng tâm: Add/move/reorder/nest/duplicate/delete.
- Hoàn thành đúng trọng tâm: fresh IDs.
- Hoàn thành đúng trọng tâm: parent/depth/count/cycle guards.
...
1. Cập nhật task `T100` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T100` trong message và chỉ stage file thuộc task.
