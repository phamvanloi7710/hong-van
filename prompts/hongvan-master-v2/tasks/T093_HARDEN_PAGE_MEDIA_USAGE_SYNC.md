# T093 — Làm cứng đồng bộ usage Page Builder
...
- **Phụ thuộc:** `T092`
- **File queue:** `tasks/T093_HARDEN_PAGE_MEDIA_USAGE_SYNC.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng đồng bộ usage Page Builder phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Sync draft/version refs trong transaction.
2. release stale refs.
3. published ownership không mất.
...
- Hoàn thành đúng trọng tâm: Sync draft/version refs trong transaction.
- Hoàn thành đúng trọng tâm: release stale refs.
- Hoàn thành đúng trọng tâm: published ownership không mất.
...
1. Cập nhật task `T093` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T093` trong message và chỉ stage file thuộc task.
