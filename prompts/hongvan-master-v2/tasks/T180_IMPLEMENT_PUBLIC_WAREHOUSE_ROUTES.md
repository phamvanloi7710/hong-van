# T180 — Triển khai public listing và detail Warehouse


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P37,P31`
- **Phụ thuộc:** `T179`
- **File queue:** `tasks/T180_IMPLEMENT_PUBLIC_WAREHOUSE_ROUTES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai public listing và detail Warehouse phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Published list/detail.
2. locale/canonical/media/facilities/services/empty state/cache.
...
- Hoàn thành đúng trọng tâm: Published list/detail.
- Hoàn thành đúng trọng tâm: locale/canonical/media/facilities/services/empty state/cache.
...
1. Cập nhật task `T180` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T180` trong message và chỉ stage file thuộc task.
