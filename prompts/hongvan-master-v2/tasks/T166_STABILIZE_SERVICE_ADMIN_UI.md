...
- **Phụ thuộc:** `T165`
- **File queue:** `tasks/T166_STABILIZE_SERVICE_ADMIN_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular Services phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Tabs/dialogs/media/CTA/status.
2. permissions/i18n/responsive/build.
...
- Hoàn thành đúng trọng tâm: Tabs/dialogs/media/CTA/status.
- Hoàn thành đúng trọng tâm: permissions/i18n/responsive/build.
...
1. Cập nhật task `T166` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T166` trong message và chỉ stage file thuộc task.
