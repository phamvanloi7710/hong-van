# T040 — Làm cứng permission override và Super Admin


## Metadata

- **Giai đoạn:** C — Authentication, RBAC, Preferences, Settings, Localization và Audit
- **Bao phủ prompt gốc:** `P11`
- **Phụ thuộc:** `T039`
- **File queue:** `tasks/T040_HARDEN_PERMISSION_OVERRIDES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng permission override và Super Admin phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
```powershell
rg -n "permission:|Gate::|Policy|hasPermission|permissionGuard" BackEnd Admin docs scripts docker 2>$null
```
...
1. Deny/allow override có precedence rõ.
2. bootstrap super admin local an toàn.
3. không seed credential production.
...
- Authorization được kiểm tra ở route/service/policy, không chỉ ẩn nút Angular.
- Deny-by-default và permission namespace nhất quán.
- Không cho bulk action vượt phạm vi resource được phép.
- Hoàn thành đúng trọng tâm: Deny/allow override có precedence rõ.
- Hoàn thành đúng trọng tâm: bootstrap super admin local an toàn.
- Hoàn thành đúng trọng tâm: không seed credential production.
...
cd BackEnd; php artisan test --filter=Identity
cd Admin; npm test -- --run identity
...
1. Cập nhật task `T040` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T040` trong message và chỉ stage file thuộc task.
