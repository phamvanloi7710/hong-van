# T084 — Làm cứng PageDocumentValidator
...
- **Phụ thuộc:** `T083`
- **File queue:** `tasks/T084_HARDEN_PAGE_VALIDATOR.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng PageDocumentValidator phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Path-specific errors.
2. unknown type/version/duplicate ID/cycle/invalid child/reference/script payload bị chặn.
...
- Hoàn thành đúng trọng tâm: Path-specific errors.
- Hoàn thành đúng trọng tâm: unknown type/version/duplicate ID/cycle/invalid child/reference/script payload bị chặn.
...
1. Cập nhật task `T084` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T084` trong message và chỉ stage file thuộc task.
