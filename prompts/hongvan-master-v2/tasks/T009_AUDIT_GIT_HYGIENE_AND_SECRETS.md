# T009 — Rà Git hygiene, file ignore và lịch sử secret


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P03,P50`
- **Phụ thuộc:** `T008`
- **File queue:** `tasks/T009_AUDIT_GIT_HYGIENE_AND_SECRETS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà Git hygiene, file ignore và lịch sử secret phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Kiểm tra generated/build/cache/upload/template ignore.
2. chạy secret scan hiện tại và lịch sử theo policy.
3. không xóa lịch sử hoặc force push trong task này.
...
- Hoàn thành đúng trọng tâm: Kiểm tra generated/build/cache/upload/template ignore.
- Hoàn thành đúng trọng tâm: chạy secret scan hiện tại và lịch sử theo policy.
- Hoàn thành đúng trọng tâm: không xóa lịch sử hoặc force push trong task này.
...
1. Cập nhật task `T009` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T009` trong message và chỉ stage file thuộc task.
