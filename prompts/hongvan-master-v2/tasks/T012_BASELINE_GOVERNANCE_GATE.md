# T012 — Cổng nghiệm thu baseline và governance
...
- **Phụ thuộc:** `T011`
- **File queue:** `tasks/T012_BASELINE_GOVERNANCE_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng nghiệm thu baseline và governance phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Chạy lại toàn kiểm tra T001-T011.
2. khóa snapshot HEAD và trạng thái queue.
3. chỉ mở T013 khi không còn mâu thuẫn baseline chưa giải quyết.
...
- Hoàn thành đúng trọng tâm: Chạy lại toàn kiểm tra T001-T011.
- Hoàn thành đúng trọng tâm: khóa snapshot HEAD và trạng thái queue.
- Hoàn thành đúng trọng tâm: chỉ mở T013 khi không còn mâu thuẫn baseline chưa giải quyết.
...
1. Cập nhật task `T012` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T012` trong message và chỉ stage file thuộc task.
