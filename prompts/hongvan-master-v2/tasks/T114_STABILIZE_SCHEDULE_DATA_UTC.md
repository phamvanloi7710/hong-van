# T114 — Ổn định dữ liệu lịch publish theo UTC
...
- **Phụ thuộc:** `T113`
- **File queue:** `tasks/T114_STABILIZE_SCHEDULE_DATA_UTC.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định dữ liệu lịch publish theo UTC phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Typed ISO timezone input.
2. store UTC.
3. active schedule uniqueness.
4. bind immutable revision/checksum.
...
- Hoàn thành đúng trọng tâm: Typed ISO timezone input.
- Hoàn thành đúng trọng tâm: store UTC.
- Hoàn thành đúng trọng tâm: active schedule uniqueness.
- Hoàn thành đúng trọng tâm: bind immutable revision/checksum.
...
1. Cập nhật task `T114` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T114` trong message và chỉ stage file thuộc task.
