# T158 — Ổn định manager và API Crop Solutions
...
- **Phụ thuộc:** `T157`
- **File queue:** `tasks/T158_STABILIZE_CROP_MANAGER_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định manager và API Crop Solutions phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD hierarchy/stages/solutions.
2. policy/audit/cache.
3. allowlisted query.
...
- Hoàn thành đúng trọng tâm: CRUD hierarchy/stages/solutions.
- Hoàn thành đúng trọng tâm: policy/audit/cache.
- Hoàn thành đúng trọng tâm: allowlisted query.
...
1. Cập nhật task `T158` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T158` trong message và chỉ stage file thuộc task.
