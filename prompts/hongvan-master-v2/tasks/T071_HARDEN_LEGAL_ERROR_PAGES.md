# T071 — Làm cứng legal, 404 và 500 pages
...
- **Phụ thuộc:** `T070`
- **File queue:** `tasks/T071_HARDEN_LEGAL_ERROR_PAGES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng legal, 404 và 500 pages phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Privacy/terms localized.
2. error không rò debug.
3. canonical/noindex phù hợp.
...
- Hoàn thành đúng trọng tâm: Privacy/terms localized.
- Hoàn thành đúng trọng tâm: error không rò debug.
- Hoàn thành đúng trọng tâm: canonical/noindex phù hợp.
...
1. Cập nhật task `T071` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T071` trong message và chỉ stage file thuộc task.
