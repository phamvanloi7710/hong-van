# T131 — Thiết kế schema versioned Global Regions
...
- **Phụ thuộc:** `T130`
- **File queue:** `tasks/T131_DESIGN_GLOBAL_REGION_SCHEMA.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Thiết kế schema versioned Global Regions phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Header/footer/topbar reusable documents.
2. draft/published versions/pointers.
3. immutable published/audit.
...
- Hoàn thành đúng trọng tâm: Header/footer/topbar reusable documents.
- Hoàn thành đúng trọng tâm: draft/published versions/pointers.
- Hoàn thành đúng trọng tâm: immutable published/audit.
...
1. Cập nhật task `T131` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T131` trong message và chỉ stage file thuộc task.
