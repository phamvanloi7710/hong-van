# T065 — Tái kiểm định parity với StayHub Media


## Metadata

- **Giai đoạn:** D — Admin shell và Media Manager
- **Bao phủ prompt gốc:** `P17`
- **Phụ thuộc:** `T064`
- **File queue:** `tasks/T065_RECERTIFY_STAYHUB_MEDIA_PARITY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái kiểm định parity với StayHub Media phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Http/Controllers/Api/V1/Media/`
- `BackEnd/config/media.php`
- `Admin/src/app/features/media/`
...
```powershell
rg -n "MediaUpload|variant|visibility|is_locked|usage|trash|restore|content" BackEnd Admin docs scripts docker 2>$null
```
...
1. Lập fresh inventory source read-only.
2. parity workflow folder/lock/visibility/upload/editor.
3. ghi intentional differences.
...
- MIME được phát hiện từ nội dung, path do server sinh, SVG/executable bị chặn.
- File đang được tham chiếu không bị xóa vĩnh viễn.
- Streaming không lộ storage path và luôn có nosniff/authorization phù hợp.
- Hoàn thành đúng trọng tâm: Lập fresh inventory source read-only.
- Hoàn thành đúng trọng tâm: parity workflow folder/lock/visibility/upload/editor.
- Hoàn thành đúng trọng tâm: ghi intentional differences.
...
1. Cập nhật task `T065` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T065` trong message và chỉ stage file thuộc task.
