# T201 — Triển khai public galleries, projects, blocks và SEO


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P40,P31,P42`
- **Phụ thuộc:** `T200`
- **File queue:** `tasks/T201_IMPLEMENT_PUBLIC_SHOWCASE_BLOCKS_SEO.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai public galleries, projects, blocks và SEO phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/Showcase/`
- `BackEnd/app/Models/`
- `BackEnd/app/Http/Controllers/Api/V1/Showcase/`
- `Admin/src/app/features/showcase/`
...
```powershell
rg -n "gallery|partner|certification|project|document_visibility|media\ usage" BackEnd Admin docs scripts docker 2>$null
```
...
1. Listings/detail/lightbox/doc visibility.
2. partner/cert/project blocks.
3. meta/schema/cache.
...
- Tài liệu chứng nhận private không bị lộ qua public API/URL.
- Media usage và alt/caption localized được đồng bộ.
- Không tạo dữ liệu thành tích/chứng nhận giả trong seeder public.
- Hoàn thành đúng trọng tâm: Listings/detail/lightbox/doc visibility.
- Hoàn thành đúng trọng tâm: partner/cert/project blocks.
- Hoàn thành đúng trọng tâm: meta/schema/cache.
...
cd Admin; npm test -- --run showcase
...
1. Cập nhật task `T201` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T201` trong message và chỉ stage file thuộc task.
