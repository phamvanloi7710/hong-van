# T192 — Làm cứng RichText sanitizer Posts


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P39,P15`
- **Phụ thuộc:** `T191`
- **File queue:** `tasks/T192_HARDEN_POST_RICH_TEXT.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng RichText sanitizer Posts phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Allowed tags/attrs/URLs.
2. strip scripts/events/styles nguy hiểm.
3. sanitize at save/render/import.
...
- Hoàn thành đúng trọng tâm: Allowed tags/attrs/URLs.
- Hoàn thành đúng trọng tâm: strip scripts/events/styles nguy hiểm.
- Hoàn thành đúng trọng tâm: sanitize at save/render/import.
...
1. Cập nhật task `T192` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T192` trong message và chỉ stage file thuộc task.
