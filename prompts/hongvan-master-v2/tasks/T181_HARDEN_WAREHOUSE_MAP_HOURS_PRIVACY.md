# T181 — Làm cứng bản đồ, giờ mở cửa và privacy


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P37`
- **Phụ thuộc:** `T180`
- **File queue:** `tasks/T181_HARDEN_WAREHOUSE_MAP_HOURS_PRIVACY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng bản đồ, giờ mở cửa và privacy phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/Warehouses/`
- `BackEnd/app/Models/Warehouse*.php`
- `BackEnd/app/Http/Controllers/Api/V1/Warehouses/`
- `Admin/src/app/features/warehouses/`
...
```powershell
rg -n "warehouse|facility|service|request|map|privacy" BackEnd Admin docs scripts docker 2>$null
```
...
1. Chỉ public coordinates/URL được phép.
2. no secret exact location nếu private.
3. hours/timezone/accessibility.
...
- Không mở rộng thành WMS tồn kho/inbound/outbound ngoài phạm vi.
- Thông tin bản đồ tuân thủ privacy và setting công khai.
- Request mapping vào lead thống nhất và không lưu dữ liệu nhạy cảm thừa.
- Hoàn thành đúng trọng tâm: Chỉ public coordinates/URL được phép.
- Hoàn thành đúng trọng tâm: no secret exact location nếu private.
- Hoàn thành đúng trọng tâm: hours/timezone/accessibility.
...
cd Admin; npm test -- --run warehouses
...
1. Cập nhật task `T181` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T181` trong message và chỉ stage file thuộc task.
