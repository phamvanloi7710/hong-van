# T099 — Ổn định models và data-access Page Builder Angular
...
- **Phụ thuộc:** `T098`
- **File queue:** `tasks/T099_STABILIZE_EDITOR_DATA_MODELS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định models và data-access Page Builder Angular phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Strict typed API envelopes/records.
2. không any.
3. server registry là nguồn chân lý.
...
- Hoàn thành đúng trọng tâm: Strict typed API envelopes/records.
- Hoàn thành đúng trọng tâm: không any.
- Hoàn thành đúng trọng tâm: server registry là nguồn chân lý.
...
1. Cập nhật task `T099` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T099` trong message và chỉ stage file thuộc task.
