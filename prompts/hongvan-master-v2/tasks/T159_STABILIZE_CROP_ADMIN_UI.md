# T159 — Ổn định Angular Crop Solutions
...
- **Phụ thuộc:** `T158`
- **File queue:** `tasks/T159_STABILIZE_CROP_ADMIN_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular Crop Solutions phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Tabs/dialogs/timeline/media/products.
2. typed forms/permissions/i18n/responsive.
...
- Hoàn thành đúng trọng tâm: Tabs/dialogs/timeline/media/products.
- Hoàn thành đúng trọng tâm: typed forms/permissions/i18n/responsive.
...
1. Cập nhật task `T159` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T159` trong message và chỉ stage file thuộc task.
