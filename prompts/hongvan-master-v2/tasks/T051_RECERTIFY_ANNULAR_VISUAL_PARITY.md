# T051 — Tái kiểm định giao diện Annular


## Metadata

- **Giai đoạn:** D — Admin shell và Media Manager
- **Bao phủ prompt gốc:** `P06`
- **Phụ thuộc:** `T050`
- **File queue:** `tasks/T051_RECERTIFY_ANNULAR_VISUAL_PARITY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái kiểm định giao diện Annular phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. So sánh shell/dashboard/auth với Template read-only.
2. ghi parity matrix.
3. không thay template bằng UI kit khác.
...
- Hoàn thành đúng trọng tâm: So sánh shell/dashboard/auth với Template read-only.
- Hoàn thành đúng trọng tâm: ghi parity matrix.
- Hoàn thành đúng trọng tâm: không thay template bằng UI kit khác.
...
1. Cập nhật task `T051` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T051` trong message và chỉ stage file thuộc task.
