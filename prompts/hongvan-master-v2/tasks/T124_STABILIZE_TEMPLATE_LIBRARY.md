# T124 — Ổn định thư viện template P29
...
- **Phụ thuộc:** `T123`
- **File queue:** `tasks/T124_STABILIZE_TEMPLATE_LIBRARY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định thư viện template P29 phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Categories/versioned immutable templates.
2. save-as-template Material dialog.
3. create page from template và permissions.
...
- Hoàn thành đúng trọng tâm: Categories/versioned immutable templates.
- Hoàn thành đúng trọng tâm: save-as-template Material dialog.
- Hoàn thành đúng trọng tâm: create page from template và permissions.
...
1. Cập nhật task `T124` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T124` trong message và chỉ stage file thuộc task.
