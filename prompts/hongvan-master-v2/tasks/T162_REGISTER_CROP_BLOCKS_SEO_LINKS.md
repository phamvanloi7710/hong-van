# T162 — Đăng ký Crop blocks, SEO và internal links


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P24,P34,P42`
- **Phụ thuộc:** `T161`
- **File queue:** `tasks/T162_REGISTER_CROP_BLOCKS_SEO_LINKS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Đăng ký Crop blocks, SEO và internal links phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/CropSolutions/`
- `BackEnd/app/Models/Crop*.php`
- `BackEnd/app/Http/Controllers/Api/V1/CropSolutions/`
- `Admin/src/app/features/crop-solutions/`
...
```powershell
rg -n "CropSolution|stage|timeline|category|products|published" BackEnd Admin docs scripts docker 2>$null
```
...
1. Grid/timeline/solution blocks.
2. meta/schema/breadcrumb.
3. related products/services links.
...
- Cây phân cấp không có cycle và timeline có thứ tự xác định.
- Public chỉ trả nội dung published theo locale/fallback.
- Liên kết sản phẩm không gây N+1 và có cache invalidation.
- Hoàn thành đúng trọng tâm: Grid/timeline/solution blocks.
- Hoàn thành đúng trọng tâm: meta/schema/breadcrumb.
- Hoàn thành đúng trọng tâm: related products/services links.
...
cd Admin; npm test -- --run crop-solutions
...
1. Cập nhật task `T162` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T162` trong message và chỉ stage file thuộc task.
