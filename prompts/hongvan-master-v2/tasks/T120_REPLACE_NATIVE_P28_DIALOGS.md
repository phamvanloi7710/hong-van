# T120 — Thay native prompt/confirm bằng Material dialogs
...
- **Phụ thuộc:** `T119`
- **File queue:** `tasks/T120_REPLACE_NATIVE_P28_DIALOGS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Thay native prompt/confirm bằng Material dialogs phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Manual save, publish, schedule, cancel, unpublish, archive, rollback typed dialogs.
2. loading/double-submit/i18n.
...
- Hoàn thành đúng trọng tâm: Manual save, publish, schedule, cancel, unpublish, archive, rollback typed dialogs.
- Hoàn thành đúng trọng tâm: loading/double-submit/i18n.
...
1. Cập nhật task `T120` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T120` trong message và chỉ stage file thuộc task.
