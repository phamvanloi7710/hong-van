...
- **Phụ thuộc:** `T037`
- **File queue:** `tasks/T038_STABILIZE_USER_MANAGEMENT.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định quản lý user phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD, role assignment, trạng thái, session reset.
2. không cho tự khóa/xóa super admin cuối cùng.
...
- Hoàn thành đúng trọng tâm: CRUD, role assignment, trạng thái, session reset.
- Hoàn thành đúng trọng tâm: không cho tự khóa/xóa super admin cuối cùng.
...
1. Cập nhật task `T038` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T038` trong message và chỉ stage file thuộc task.
