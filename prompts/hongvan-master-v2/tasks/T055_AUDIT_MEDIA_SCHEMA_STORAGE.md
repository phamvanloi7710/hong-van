# T055 — Rà schema và storage Media
...
- **Phụ thuộc:** `T054`
- **File queue:** `tasks/T055_AUDIT_MEDIA_SCHEMA_STORAGE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema và storage Media phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Disk/path server-owned, SHA/MIME/size/status.
2. variants/usages/operations relations.
3. comments/index/FK.
...
- Hoàn thành đúng trọng tâm: Disk/path server-owned, SHA/MIME/size/status.
- Hoàn thành đúng trọng tâm: variants/usages/operations relations.
- Hoàn thành đúng trọng tâm: comments/index/FK.
...
1. Cập nhật task `T055` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T055` trong message và chỉ stage file thuộc task.
