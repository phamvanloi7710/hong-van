# T185 — Rà schema Lead và dữ liệu nhạy cảm
...
- **Phụ thuộc:** `T184`
- **File queue:** `tasks/T185_AUDIT_LEAD_SCHEMA_ENCRYPTION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema Lead và dữ liệu nhạy cảm phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Leads/assignments/status/notes/contact/quote items.
2. encryption/immutable submission/index/comments.
...
- Hoàn thành đúng trọng tâm: Leads/assignments/status/notes/contact/quote items.
- Hoàn thành đúng trọng tâm: encryption/immutable submission/index/comments.
...
1. Cập nhật task `T185` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T185` trong message và chỉ stage file thuộc task.
