# T189 — Ổn định inbox, export và retention


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P38`
- **Phụ thuộc:** `T188`
- **File queue:** `tasks/T189_STABILIZE_LEAD_ADMIN_EXPORT_RETENTION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định inbox, export và retention phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Filters/metrics/timeline/notes/assignee.
2. CSV safe/private.
3. anonymize expired without breaking audit.
...
- Hoàn thành đúng trọng tâm: Filters/metrics/timeline/notes/assignee.
- Hoàn thành đúng trọng tâm: CSV safe/private.
- Hoàn thành đúng trọng tâm: anonymize expired without breaking audit.
...
1. Cập nhật task `T189` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T189` trong message và chỉ stage file thuộc task.
