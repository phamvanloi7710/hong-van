# T167 — Triển khai public list/detail và CTA Service


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P35,P31`
- **Phụ thuộc:** `T166`
- **File queue:** `tasks/T167_IMPLEMENT_PUBLIC_SERVICE_ROUTES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai public list/detail và CTA Service phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Http/Controllers/Api/V1/Services/`
- `Admin/src/app/features/services/`
...
```powershell
rg -n "ServiceManager|category|cta|transportation|warehouse|published" BackEnd Admin docs scripts docker 2>$null
```
...
1. Published routes/locale/canonical.
2. contact/quote link.
3. transport/warehouse explicit links.
...
- Dịch vụ chung không trùng dữ liệu chuyên ngành vận chuyển/kho.
- CTA dùng nguồn được phép và không nhận URL/script tùy ý.
- Public list/detail, SEO và Page Builder dùng cùng data source published.
- Hoàn thành đúng trọng tâm: Published routes/locale/canonical.
- Hoàn thành đúng trọng tâm: contact/quote link.
- Hoàn thành đúng trọng tâm: transport/warehouse explicit links.
...
cd Admin; npm test -- --run services
...
1. Cập nhật task `T167` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T167` trong message và chỉ stage file thuộc task.
