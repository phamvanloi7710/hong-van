# T221 — Làm cứng reports queue và Dashboard UI


## Metadata

- **Giai đoạn:** L — SEO, Analytics và Dashboard
- **Bao phủ prompt gốc:** `P45`
- **Phụ thuộc:** `T220`
- **File queue:** `tasks/T221_HARDEN_REPORTS_DASHBOARD_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng reports queue và Dashboard UI phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `Admin/src/app/features/dashboard/`
- `Admin/src/app/core/layout/admin-header/`
...
```powershell
rg -n "DashboardAggregate|visibility|notifications|ReportExport|download" BackEnd Admin docs scripts docker 2>$null
```
...
1. Private queued export/status/download expiry.
2. charts/cards loading/empty/error.
3. responsive/i18n.
...
- Metrics/report chỉ dùng dữ liệu user được phép xem.
- Export nặng chạy queue và download private/expiring.
- Notification deep-link không dẫn tới resource ngoài quyền.
- Hoàn thành đúng trọng tâm: Private queued export/status/download expiry.
- Hoàn thành đúng trọng tâm: charts/cards loading/empty/error.
- Hoàn thành đúng trọng tâm: responsive/i18n.
...
cd Admin; npm test -- --run dashboard
...
1. Cập nhật task `T221` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T221` trong message và chỉ stage file thuộc task.
