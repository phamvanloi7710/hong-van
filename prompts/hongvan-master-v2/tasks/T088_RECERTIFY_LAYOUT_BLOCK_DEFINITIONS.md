# T088 — Tái kiểm định 7 layout blocks
...
- **Phụ thuộc:** `T087`
- **File queue:** `tasks/T088_RECERTIFY_LAYOUT_BLOCK_DEFINITIONS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái kiểm định 7 layout blocks phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Section/Container/Stack/Grid/Columns/Spacer/Divider schema/default/constraints/fixtures đầy đủ.
...
- Hoàn thành đúng trọng tâm: Section/Container/Stack/Grid/Columns/Spacer/Divider schema/default/constraints/fixtures đầy đủ.
...
1. Cập nhật task `T088` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T088` trong message và chỉ stage file thuộc task.
