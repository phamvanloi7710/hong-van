# T053 — Ổn định header, breadcrumb, icon và favorites
...
- **Phụ thuộc:** `T052`
- **File queue:** `tasks/T053_STABILIZE_HEADER_BREADCRUMB_FAVORITES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định header, breadcrumb, icon và favorites phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Route-aware title/icon.
2. notification/favorites/theme controls.
3. không duplicated navigation state.
...
- Hoàn thành đúng trọng tâm: Route-aware title/icon.
- Hoàn thành đúng trọng tâm: notification/favorites/theme controls.
- Hoàn thành đúng trọng tâm: không duplicated navigation state.
...
1. Cập nhật task `T053` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T053` trong message và chỉ stage file thuộc task.
