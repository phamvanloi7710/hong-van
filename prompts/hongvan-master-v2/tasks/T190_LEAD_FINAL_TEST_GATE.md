# T190 — Cổng test/E2E Lead workflows
...
- **Phụ thuộc:** `T189`
- **File queue:** `tasks/T190_LEAD_FINAL_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng test/E2E Lead workflows phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. All intake variants, dedup, permission, transition, export, retention, notifications and mobile Admin pass.
...
- Hoàn thành đúng trọng tâm: All intake variants, dedup, permission, transition, export, retention, notifications and mobile Admin pass.
...
1. Cập nhật task `T190` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T190` trong message và chỉ stage file thuộc task.
