# T083 — Làm cứng BlockRegistry metadata
...
- **Phụ thuộc:** `T082`
- **File queue:** `tasks/T083_HARDEN_BLOCK_REGISTRY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng BlockRegistry metadata phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Typed type/version/labels/category/icon/schema/default/parent-child.
2. API không lộ class/view/path.
...
- Hoàn thành đúng trọng tâm: Typed type/version/labels/category/icon/schema/default/parent-child.
- Hoàn thành đúng trọng tâm: API không lộ class/view/path.
...
1. Cập nhật task `T083` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T083` trong message và chỉ stage file thuộc task.
