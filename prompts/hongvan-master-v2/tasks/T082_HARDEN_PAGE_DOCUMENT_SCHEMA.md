# T082 — Làm cứng PageDocument schema và checksum


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P21`
- **Phụ thuộc:** `T081`
- **File queue:** `tasks/T082_HARDEN_PAGE_DOCUMENT_SCHEMA.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng PageDocument schema và checksum phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Size/depth/block limits.
2. allowlisted root/settings/block fields.
3. canonical checksum deterministic.
...
- Hoàn thành đúng trọng tâm: Size/depth/block limits.
- Hoàn thành đúng trọng tâm: allowlisted root/settings/block fields.
- Hoàn thành đúng trọng tâm: canonical checksum deterministic.
...
1. Cập nhật task `T082` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T082` trong message và chỉ stage file thuộc task.
