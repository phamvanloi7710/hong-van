# T046 — Làm cứng cấu hình bí mật


## Metadata

- **Giai đoạn:** C — Authentication, RBAC, Preferences, Settings, Localization và Audit
- **Bao phủ prompt gốc:** `P13,P15`
- **Phụ thuộc:** `T045`
- **File queue:** `tasks/T046_HARDEN_SECRET_SETTINGS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng cấu hình bí mật phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Secret lưu encrypted hoặc env reference.
2. API chỉ has_value.
3. không xuất ciphertext/plaintext/log.
...
- Hoàn thành đúng trọng tâm: Secret lưu encrypted hoặc env reference.
- Hoàn thành đúng trọng tâm: API chỉ has_value.
- Hoàn thành đúng trọng tâm: không xuất ciphertext/plaintext/log.
...
1. Cập nhật task `T046` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T046` trong message và chỉ stage file thuộc task.
