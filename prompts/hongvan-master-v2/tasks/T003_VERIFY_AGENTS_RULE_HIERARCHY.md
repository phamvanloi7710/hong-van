# T003 — Kiểm tra hệ thống AGENTS.md và phạm vi quy tắc


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P00,P03`
- **Phụ thuộc:** `T002`
- **File queue:** `tasks/T003_VERIFY_AGENTS_RULE_HIERARCHY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Kiểm tra hệ thống AGENTS.md và phạm vi quy tắc phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Tìm toàn bộ AGENTS.md.
2. kiểm tra quy tắc thư mục con không làm yếu quy tắc gốc.
3. bổ sung quy tắc còn thiếu cho các vùng quan trọng nếu cần.
...
- Hoàn thành đúng trọng tâm: Tìm toàn bộ AGENTS.md.
- Hoàn thành đúng trọng tâm: kiểm tra quy tắc thư mục con không làm yếu quy tắc gốc.
- Hoàn thành đúng trọng tâm: bổ sung quy tắc còn thiếu cho các vùng quan trọng nếu cần.
...
1. Cập nhật task `T003` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T003` trong message và chỉ stage file thuộc task.
