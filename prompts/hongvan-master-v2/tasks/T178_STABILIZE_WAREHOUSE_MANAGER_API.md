# T178 — Ổn định manager và API Warehouse
...
- **Phụ thuộc:** `T177`
- **File queue:** `tasks/T178_STABILIZE_WAREHOUSE_MANAGER_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định manager và API Warehouse phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD/publish/featured/facility/service.
2. policy/audit/cache/query/media.
...
- Hoàn thành đúng trọng tâm: CRUD/publish/featured/facility/service.
- Hoàn thành đúng trọng tâm: policy/audit/cache/query/media.
...
1. Cập nhật task `T178` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T178` trong message và chỉ stage file thuộc task.
