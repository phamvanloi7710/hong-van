# T044 — Ổn định menu yêu thích và điều hướng Annular


## Metadata

- **Giai đoạn:** C — Authentication, RBAC, Preferences, Settings, Localization và Audit
- **Bao phủ prompt gốc:** `P12,P17`
- **Phụ thuộc:** `T043`
- **File queue:** `tasks/T044_STABILIZE_FAVORITE_NAVIGATION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định menu yêu thích và điều hướng Annular phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Multi-select, thứ tự, icon tim đứng trước shortcut.
2. permission thay đổi loại shortcut không còn quyền.
...
- Hoàn thành đúng trọng tâm: Multi-select, thứ tự, icon tim đứng trước shortcut.
- Hoàn thành đúng trọng tâm: permission thay đổi loại shortcut không còn quyền.
...
1. Cập nhật task `T044` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T044` trong message và chỉ stage file thuộc task.
