# T134 — Render header/footer/global regions trên public
...
- **Phụ thuộc:** `T133`
- **File queue:** `tasks/T134_RENDER_HEADER_FOOTER_REGIONS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Render header/footer/global regions trên public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Published region + menu per locale.
2. fallback/default.
3. semantic/a11y.
4. no draft leakage.
...
- Hoàn thành đúng trọng tâm: Published region + menu per locale.
- Hoàn thành đúng trọng tâm: fallback/default.
- Hoàn thành đúng trọng tâm: semantic/a11y.
- Hoàn thành đúng trọng tâm: no draft leakage.
...
1. Cập nhật task `T134` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T134` trong message và chỉ stage file thuộc task.
