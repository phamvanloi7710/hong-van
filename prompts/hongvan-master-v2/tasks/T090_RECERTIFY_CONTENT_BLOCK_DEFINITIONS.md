# T090 — Tái kiểm định content blocks
...
- **Phụ thuộc:** `T089`
- **File queue:** `tasks/T090_RECERTIFY_CONTENT_BLOCK_DEFINITIONS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái kiểm định content blocks phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Heading/RichText/Button/Icon/List/Quote/Table/Badge/Card/Image/ImageText/Gallery/Video/Logo/FAQ schemas và fixtures.
...
- Hoàn thành đúng trọng tâm: Heading/RichText/Button/Icon/List/Quote/Table/Badge/Card/Image/ImageText/Gallery/Video/Logo/FAQ schemas và fixtures.
...
1. Cập nhật task `T090` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T090` trong message và chỉ stage file thuộc task.
