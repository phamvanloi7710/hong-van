# T017 — Cưỡng chế comment cho mọi bảng và cột
...
- **Phụ thuộc:** `T016`
- **File queue:** `tasks/T017_ENFORCE_DATABASE_COMMENTS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cưỡng chế comment cho mọi bảng và cột phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Quét schema MySQL thực và migrations.
2. thêm/sửa migration mới để duy trì comments.
3. không chỉnh migration production cũ trái policy nếu cần migration bổ sung.
...
- Hoàn thành đúng trọng tâm: Quét schema MySQL thực và migrations.
- Hoàn thành đúng trọng tâm: thêm/sửa migration mới để duy trì comments.
- Hoàn thành đúng trọng tâm: không chỉnh migration production cũ trái policy nếu cần migration bổ sung.
...
1. Cập nhật task `T017` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T017` trong message và chỉ stage file thuộc task.
