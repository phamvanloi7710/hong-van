# T238 — Audit test, security, CI và operations
...
- **Phụ thuộc:** `T237`
- **File queue:** `tasks/T238_AUDIT_TEST_SECURITY_CI_OPERATIONS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Audit test, security, CI và operations phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Detect skipped/flaky/weak tests.
2. rerun scans/pipeline.
3. review backup/restore/monitoring/runbooks and unresolved risks.
...
- Hoàn thành đúng trọng tâm: Detect skipped/flaky/weak tests.
- Hoàn thành đúng trọng tâm: rerun scans/pipeline.
- Hoàn thành đúng trọng tâm: review backup/restore/monitoring/runbooks and unresolved risks.
...
1. Cập nhật task `T238` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T238` trong message và chỉ stage file thuộc task.
