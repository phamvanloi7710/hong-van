# T135 — Thiết kế registry route public
...
- **Phụ thuộc:** `T134`
- **File queue:** `tasks/T135_DESIGN_PUBLIC_ROUTE_REGISTRY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Thiết kế registry route public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Page/entity types, locale prefixes, reserved paths, canonical route names.
2. không catch-all nuốt sitemap/admin/api.
...
- Hoàn thành đúng trọng tâm: Page/entity types, locale prefixes, reserved paths, canonical route names.
- Hoàn thành đúng trọng tâm: không catch-all nuốt sitemap/admin/api.
...
1. Cập nhật task `T135` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T135` trong message và chỉ stage file thuộc task.
