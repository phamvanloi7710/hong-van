# T087 — Chuẩn hóa cache key và dependency tags


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P21,P24`
- **Phụ thuộc:** `T086`
- **File queue:** `tasks/T087_STANDARDIZE_PAGE_CACHE_KEYS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Chuẩn hóa cache key và dependency tags phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Key gồm page/locale/version/theme.
2. data source/media/menu/theme dependencies.
3. invalidation xác định.
...
- Hoàn thành đúng trọng tâm: Key gồm page/locale/version/theme.
- Hoàn thành đúng trọng tâm: data source/media/menu/theme dependencies.
- Hoàn thành đúng trọng tâm: invalidation xác định.
...
1. Cập nhật task `T087` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T087` trong message và chỉ stage file thuộc task.
