# T146 — Ổn định Product manager và Admin API
...
- **Phụ thuộc:** `T145`
- **File queue:** `tasks/T146_STABILIZE_PRODUCT_MANAGER_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Product manager và Admin API phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Thin controller/FormRequest/manager/resource/policy.
2. audit/cache.
3. allowlisted filters/sorts.
...
- Hoàn thành đúng trọng tâm: Thin controller/FormRequest/manager/resource/policy.
- Hoàn thành đúng trọng tâm: audit/cache.
- Hoàn thành đúng trọng tâm: allowlisted filters/sorts.
...
1. Cập nhật task `T146` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T146` trong message và chỉ stage file thuộc task.
