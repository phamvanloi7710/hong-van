# T157 — Rà schema và hierarchy Crop
...
- **Phụ thuộc:** `T156`
- **File queue:** `tasks/T157_AUDIT_CROP_SCHEMA_HIERARCHY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema và hierarchy Crop phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Categories/crops/stages/solutions/translations/products.
2. comments/FK/order/cycle/status.
...
- Hoàn thành đúng trọng tâm: Categories/crops/stages/solutions/translations/products.
- Hoàn thành đúng trọng tâm: comments/FK/order/cycle/status.
...
1. Cập nhật task `T157` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T157` trong message và chỉ stage file thuộc task.
