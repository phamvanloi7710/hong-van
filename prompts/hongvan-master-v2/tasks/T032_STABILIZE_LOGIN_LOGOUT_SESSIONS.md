# T032 — Ổn định login, logout và vòng đời session


## Metadata

- **Giai đoạn:** C — Authentication, RBAC, Preferences, Settings, Localization và Audit
- **Bao phủ prompt gốc:** `P10`
- **Phụ thuộc:** `T031`
- **File queue:** `tasks/T032_STABILIZE_LOGIN_LOGOUT_SESSIONS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định login, logout và vòng đời session phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Regenerate session sau login.
2. logout revoke đúng.
3. blocked/inactive account không đăng nhập và không giữ session cũ.
...
- Hoàn thành đúng trọng tâm: Regenerate session sau login.
- Hoàn thành đúng trọng tâm: logout revoke đúng.
- Hoàn thành đúng trọng tâm: blocked/inactive account không đăng nhập và không giữ session cũ.
...
1. Cập nhật task `T032` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T032` trong message và chỉ stage file thuộc task.
