# T210 — Ổn định Angular SEO management
...
- **Phụ thuộc:** `T209`
- **File queue:** `tasks/T210_STABILIZE_SEO_ADMIN.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular SEO management phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Entity search/edit/preview/validation.
2. permissions/i18n.
3. social image picker.
4. no raw arbitrary tags.
...
- Hoàn thành đúng trọng tâm: Entity search/edit/preview/validation.
- Hoàn thành đúng trọng tâm: permissions/i18n.
- Hoàn thành đúng trọng tâm: social image picker.
- Hoàn thành đúng trọng tâm: no raw arbitrary tags.
...
1. Cập nhật task `T210` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T210` trong message và chỉ stage file thuộc task.
