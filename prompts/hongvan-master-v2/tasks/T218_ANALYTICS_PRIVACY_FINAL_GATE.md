# T218 — Cổng privacy và test Analytics
...
- **Phụ thuộc:** `T217`
- **File queue:** `tasks/T218_ANALYTICS_PRIVACY_FINAL_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng privacy và test Analytics phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. No-cookie before consent, revoke stops scripts, CSP tests, invalid setting rejection, final routes/layouts covered.
...
- Hoàn thành đúng trọng tâm: No-cookie before consent, revoke stops scripts, CSP tests, invalid setting rejection, final routes/layouts covered.
...
1. Cập nhật task `T218` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T218` trong message và chỉ stage file thuộc task.
