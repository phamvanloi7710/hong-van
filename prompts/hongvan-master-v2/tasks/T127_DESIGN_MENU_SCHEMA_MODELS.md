# T127 — Thiết kế và triển khai schema Menu
...
- **Phụ thuộc:** `T126`
- **File queue:** `tasks/T127_DESIGN_MENU_SCHEMA_MODELS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Thiết kế và triển khai schema Menu phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Menus/items/translations/public IDs.
2. parent/order/status/target.
3. comments/FK/index/rollback.
...
- Hoàn thành đúng trọng tâm: Menus/items/translations/public IDs.
- Hoàn thành đúng trọng tâm: parent/order/status/target.
- Hoàn thành đúng trọng tâm: comments/FK/index/rollback.
...
1. Cập nhật task `T127` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T127` trong message và chỉ stage file thuộc task.
