# T020 — Chuẩn hóa response envelope và pagination


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P09`
- **Phụ thuộc:** `T019`
- **File queue:** `tasks/T020_STANDARDIZE_API_ENVELOPE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Chuẩn hóa response envelope và pagination phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Kiểm tra ApiResponse success/error/paginated.
2. binary/download/stream không bị bọc sai.
3. metadata request_id và pagination nhất quán.
...
- Hoàn thành đúng trọng tâm: Kiểm tra ApiResponse success/error/paginated.
- Hoàn thành đúng trọng tâm: binary/download/stream không bị bọc sai.
- Hoàn thành đúng trọng tâm: metadata request_id và pagination nhất quán.
...
1. Cập nhật task `T020` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T020` trong message và chỉ stage file thuộc task.
