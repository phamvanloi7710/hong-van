# T139 — Hoàn thiện các core page public
...
- **Phụ thuộc:** `T138`
- **File queue:** `tasks/T139_IMPLEMENT_CORE_PUBLIC_PAGES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Hoàn thiện các core page public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Home/contact/about/legal/content contracts.
2. Page Builder or dedicated renderer decision rõ.
3. vi/en/zh.
...
- Hoàn thành đúng trọng tâm: Home/contact/about/legal/content contracts.
- Hoàn thành đúng trọng tâm: Page Builder or dedicated renderer decision rõ.
- Hoàn thành đúng trọng tâm: vi/en/zh.
...
1. Cập nhật task `T139` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T139` trong message và chỉ stage file thuộc task.
