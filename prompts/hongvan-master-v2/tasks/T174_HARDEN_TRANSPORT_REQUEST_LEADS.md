# T174 — Làm cứng request vận chuyển và Lead integration


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P25,P36,P38`
- **Phụ thuộc:** `T173`
- **File queue:** `tasks/T174_HARDEN_TRANSPORT_REQUEST_LEADS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng request vận chuyển và Lead integration phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Pickup/delivery/cargo/weight/vehicle/date/contact.
2. consent/idempotency/dedup/mapping/notifications.
...
- Hoàn thành đúng trọng tâm: Pickup/delivery/cargo/weight/vehicle/date/contact.
- Hoàn thành đúng trọng tâm: consent/idempotency/dedup/mapping/notifications.
...
1. Cập nhật task `T174` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T174` trong message và chỉ stage file thuộc task.
