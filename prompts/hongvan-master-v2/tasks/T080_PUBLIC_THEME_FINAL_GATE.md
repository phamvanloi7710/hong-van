# T080 — Cổng cuối Public foundation và Theme


## Metadata

- **Giai đoạn:** E — Public Blade foundation và Theme Studio
- **Bao phủ prompt gốc:** `P18-P20`
- **Phụ thuộc:** `T079`
- **File queue:** `tasks/T080_PUBLIC_THEME_FINAL_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng cuối Public foundation và Theme phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Backend/Angular/build/runtime Chrome pass.
2. source templates unchanged.
3. SEO/accessibility baseline xanh.
...
- Hoàn thành đúng trọng tâm: Backend/Angular/build/runtime Chrome pass.
- Hoàn thành đúng trọng tâm: source templates unchanged.
- Hoàn thành đúng trọng tâm: SEO/accessibility baseline xanh.
...
1. Cập nhật task `T080` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T080` trong message và chỉ stage file thuộc task.
