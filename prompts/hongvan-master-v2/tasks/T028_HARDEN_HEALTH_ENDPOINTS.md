# T028 — Làm cứng health và system ping
...
- **Phụ thuộc:** `T027`
- **File queue:** `tasks/T028_HARDEN_HEALTH_ENDPOINTS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng health và system ping phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Public ping chỉ trạng thái ứng dụng.
2. admin health có auth/permission.
3. không lộ version, DB host hoặc dependency secret.
...
- Hoàn thành đúng trọng tâm: Public ping chỉ trạng thái ứng dụng.
- Hoàn thành đúng trọng tâm: admin health có auth/permission.
- Hoàn thành đúng trọng tâm: không lộ version, DB host hoặc dependency secret.
...
1. Cập nhật task `T028` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T028` trong message và chỉ stage file thuộc task.
