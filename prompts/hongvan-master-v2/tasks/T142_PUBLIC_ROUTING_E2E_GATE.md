# T142 — Cổng E2E Menu, Regions và Public Routing


## Metadata

- **Giai đoạn:** H — Menus, Global Regions và Public Routing
- **Bao phủ prompt gốc:** `P30,P31`
- **Phụ thuộc:** `T141`
- **File queue:** `tasks/T142_PUBLIC_ROUTING_E2E_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng E2E Menu, Regions và Public Routing phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/PageBuilder/`
- `BackEnd/app/Domain/PublicSite/`
- `BackEnd/app/Models/`
...
```powershell
rg -n "Menu|GlobalRegion|localized\ slug|published_version_id|route\ collision|canonical" BackEnd Admin docs scripts docker 2>$null
```
...
1. Crawl vi/en/zh core pages.
2. publish/rollback/menu update visible.
3. 404/draft protection/canonical/assets/console clean.
...
- Draft/private content không truy cập được qua public URL.
- Slug collision và locale fallback được xử lý xác định, không route shadowing.
- Header/footer/global region dùng version published bất biến và cache đúng.
- Hoàn thành đúng trọng tâm: Crawl vi/en/zh core pages.
- Hoàn thành đúng trọng tâm: publish/rollback/menu update visible.
- Hoàn thành đúng trọng tâm: 404/draft protection/canonical/assets/console clean.
...
cd BackEnd; php artisan test --filter=PublicRouting
...
1. Cập nhật task `T142` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T142` trong message và chỉ stage file thuộc task.
