# T140 — Cô lập draft, preview và published public
...
- **Phụ thuộc:** `T139`
- **File queue:** `tasks/T140_ISOLATE_DRAFT_PREVIEW_PUBLISHED.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cô lập draft, preview và published public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Signed preview owner/noindex.
2. draft never public.
3. published immutable pointer.
4. cache separation.
...
- Hoàn thành đúng trọng tâm: Signed preview owner/noindex.
- Hoàn thành đúng trọng tâm: draft never public.
- Hoàn thành đúng trọng tâm: published immutable pointer.
- Hoàn thành đúng trọng tâm: cache separation.
...
1. Cập nhật task `T140` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T140` trong message và chỉ stage file thuộc task.
