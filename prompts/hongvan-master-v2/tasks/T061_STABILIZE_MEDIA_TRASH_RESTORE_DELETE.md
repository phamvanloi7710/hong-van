# T061 — Ổn định trash, restore và xóa vĩnh viễn


## Metadata

- **Giai đoạn:** D — Admin shell và Media Manager
- **Bao phủ prompt gốc:** `P16,P17`
- **Phụ thuộc:** `T060`
- **File queue:** `tasks/T061_STABILIZE_MEDIA_TRASH_RESTORE_DELETE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định trash, restore và xóa vĩnh viễn phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Lifecycle/audit/authorization.
2. cleanup original+variants qua Filesystem.
3. usage guard.
...
- Hoàn thành đúng trọng tâm: Lifecycle/audit/authorization.
- Hoàn thành đúng trọng tâm: cleanup original+variants qua Filesystem.
- Hoàn thành đúng trọng tâm: usage guard.
...
1. Cập nhật task `T061` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T061` trong message và chỉ stage file thuộc task.
