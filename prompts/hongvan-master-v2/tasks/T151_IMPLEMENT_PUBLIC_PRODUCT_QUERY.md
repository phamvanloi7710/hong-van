# T151 — Triển khai query catalog public
...
- **Phụ thuộc:** `T150`
- **File queue:** `tasks/T151_IMPLEMENT_PUBLIC_PRODUCT_QUERY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai query catalog public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Published-only, locale fallback, eager media/taxonomy/price.
2. filter/sort/pagination allowlist.
3. cache.
...
- Hoàn thành đúng trọng tâm: Published-only, locale fallback, eager media/taxonomy/price.
- Hoàn thành đúng trọng tâm: filter/sort/pagination allowlist.
- Hoàn thành đúng trọng tâm: cache.
...
1. Cập nhật task `T151` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T151` trong message và chỉ stage file thuộc task.
