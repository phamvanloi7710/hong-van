# T224 — Tái chứng nhận QA Backend toàn hệ thống


## Metadata

- **Giai đoạn:** M — Seeders, QA, CI, Deployment, Operations, Security, UAT và Handover
- **Bao phủ prompt gốc:** `P48`
- **Phụ thuộc:** `T223`
- **File queue:** `tasks/T224_BACKEND_QA_RECERTIFICATION.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái chứng nhận QA Backend toàn hệ thống phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Full PHPUnit/Pint/PHPStan/migration prefix-comment/rollback.
2. fix root causes.
3. record durations/failures.
...
- Hoàn thành đúng trọng tâm: Full PHPUnit/Pint/PHPStan/migration prefix-comment/rollback.
- Hoàn thành đúng trọng tâm: fix root causes.
- Hoàn thành đúng trọng tâm: record durations/failures.
...
1. Cập nhật task `T224` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T224` trong message và chỉ stage file thuộc task.
