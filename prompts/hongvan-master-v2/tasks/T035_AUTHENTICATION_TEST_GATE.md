# T035 — Cổng kiểm thử authentication
...
- **Phụ thuộc:** `T034`
- **File queue:** `tasks/T035_AUTHENTICATION_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng kiểm thử authentication phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Test CSRF, fixation, throttle, reset token, locked user và logout.
2. E2E login/logout không console error.
...
- Hoàn thành đúng trọng tâm: Test CSRF, fixation, throttle, reset token, locked user và logout.
- Hoàn thành đúng trọng tâm: E2E login/logout không console error.
...
1. Cập nhật task `T035` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T035` trong message và chỉ stage file thuộc task.
