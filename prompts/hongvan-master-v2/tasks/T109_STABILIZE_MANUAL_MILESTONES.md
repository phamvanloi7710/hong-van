# T109 — Ổn định manual save và immutable milestones
...
- **Phụ thuộc:** `T108`
- **File queue:** `tasks/T109_STABILIZE_MANUAL_MILESTONES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định manual save và immutable milestones phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Transaction lockForUpdate, sequence unique, author/note/checksum/schema/source.
2. không tạo revision rác khi unchanged.
...
- Hoàn thành đúng trọng tâm: Transaction lockForUpdate, sequence unique, author/note/checksum/schema/source.
- Hoàn thành đúng trọng tâm: không tạo revision rác khi unchanged.
...
1. Cập nhật task `T109` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T109` trong message và chỉ stage file thuộc task.
