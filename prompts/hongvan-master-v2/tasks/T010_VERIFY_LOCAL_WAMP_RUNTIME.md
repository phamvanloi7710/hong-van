# T010 — Xác minh runtime WAMP và domain hongvan.local


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P04-P07`
- **Phụ thuộc:** `T009`
- **File queue:** `tasks/T010_VERIFY_LOCAL_WAMP_RUNTIME.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Xác minh runtime WAMP và domain hongvan.local phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Kiểm tra PHP/Node/MySQL/domain/document root.
2. chạy public/admin ping và MIME asset.
3. ghi rõ dependency nào không thể xác minh thay vì đoán.
...
- Hoàn thành đúng trọng tâm: Kiểm tra PHP/Node/MySQL/domain/document root.
- Hoàn thành đúng trọng tâm: chạy public/admin ping và MIME asset.
- Hoàn thành đúng trọng tâm: ghi rõ dependency nào không thể xác minh thay vì đoán.
...
1. Cập nhật task `T010` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T010` trong message và chỉ stage file thuộc task.
