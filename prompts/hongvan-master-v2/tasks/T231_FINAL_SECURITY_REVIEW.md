# T231 — Security review cuối toàn hệ thống


## Metadata

- **Giai đoạn:** M — Seeders, QA, CI, Deployment, Operations, Security, UAT và Handover
- **Bao phủ prompt gốc:** `P53`
- **Phụ thuộc:** `T230`
- **File queue:** `tasks/T231_FINAL_SECURITY_REVIEW.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Security review cuối toàn hệ thống phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Threat model/auth/RBAC/uploads/XSS/CSRF/SSRF/SQL/files/preview/import/export/deploy/privacy.
2. close Critical/High.
...
- Hoàn thành đúng trọng tâm: Threat model/auth/RBAC/uploads/XSS/CSRF/SSRF/SQL/files/preview/import/export/deploy/privacy.
- Hoàn thành đúng trọng tâm: close Critical/High.
...
1. Cập nhật task `T231` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T231` trong message và chỉ stage file thuộc task.
