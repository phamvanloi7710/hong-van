# T008 — Rà soát hợp đồng biến môi trường


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P03,P04`
- **Phụ thuộc:** `T007`
- **File queue:** `tasks/T008_AUDIT_ENVIRONMENT_CONTRACTS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà soát hợp đồng biến môi trường phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. So sánh .env.example với config sử dụng.
2. loại biến chết và bổ sung biến thiếu bằng placeholder an toàn.
3. không chép secret thật.
...
- Hoàn thành đúng trọng tâm: So sánh .env.example với config sử dụng.
- Hoàn thành đúng trọng tâm: loại biến chết và bổ sung biến thiếu bằng placeholder an toàn.
- Hoàn thành đúng trọng tâm: không chép secret thật.
...
1. Cập nhật task `T008` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T008` trong message và chỉ stage file thuộc task.
