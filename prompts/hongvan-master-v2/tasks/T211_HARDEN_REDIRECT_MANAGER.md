# T211 — Làm cứng redirect manager
...
- **Phụ thuộc:** `T210`
- **File queue:** `tasks/T211_HARDEN_REDIRECT_MANAGER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng redirect manager phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Exact internal path/from-to/status.
2. collision/loop/chain/open redirect prevention.
3. hit/audit if contract.
...
- Hoàn thành đúng trọng tâm: Exact internal path/from-to/status.
- Hoàn thành đúng trọng tâm: collision/loop/chain/open redirect prevention.
- Hoàn thành đúng trọng tâm: hit/audit if contract.
...
1. Cập nhật task `T211` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T211` trong message và chỉ stage file thuộc task.
