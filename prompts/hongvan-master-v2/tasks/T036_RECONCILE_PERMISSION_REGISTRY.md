# T036 — Đối chiếu registry permission với route và UI
...
- **Phụ thuộc:** `T035`
- **File queue:** `tasks/T036_RECONCILE_PERMISSION_REGISTRY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Đối chiếu registry permission với route và UI phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Liệt kê permission thực.
2. phát hiện route thiếu/mồ côi.
3. seed idempotent và label vi/en/zh.
...
- Hoàn thành đúng trọng tâm: Liệt kê permission thực.
- Hoàn thành đúng trọng tâm: phát hiện route thiếu/mồ côi.
- Hoàn thành đúng trọng tâm: seed idempotent và label vi/en/zh.
...
1. Cập nhật task `T036` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T036` trong message và chỉ stage file thuộc task.
