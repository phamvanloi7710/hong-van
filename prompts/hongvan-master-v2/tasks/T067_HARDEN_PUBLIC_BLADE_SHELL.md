# T067 — Làm cứng Blade public shell
...
- **Phụ thuộc:** `T066`
- **File queue:** `tasks/T067_HARDEN_PUBLIC_BLADE_SHELL.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng Blade public shell phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Semantic header/main/footer/skip link.
2. settings-backed company data.
3. không loopback API.
...
- Hoàn thành đúng trọng tâm: Semantic header/main/footer/skip link.
- Hoàn thành đúng trọng tâm: settings-backed company data.
- Hoàn thành đúng trọng tâm: không loopback API.
...
1. Cập nhật task `T067` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T067` trong message và chỉ stage file thuộc task.
