# T107 — Audit toàn bộ lifecycle versioning và publishing P28
...
- **Phụ thuộc:** `T106`
- **File queue:** `tasks/T107_AUDIT_P28_LIFECYCLE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Audit toàn bộ lifecycle versioning và publishing P28 phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Đối chiếu implementation với prompt/ADR/test.
2. ghi bảng autosave/milestone/publish/schedule/cancel/unpublish/archive/rollback/diff.
3. không sửa trước khi root causes rõ.
...
- Hoàn thành đúng trọng tâm: Đối chiếu implementation với prompt/ADR/test.
- Hoàn thành đúng trọng tâm: ghi bảng autosave/milestone/publish/schedule/cancel/unpublish/archive/rollback/diff.
- Hoàn thành đúng trọng tâm: không sửa trước khi root causes rõ.
...
1. Cập nhật task `T107` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T107` trong message và chỉ stage file thuộc task.
