# T206 — Triển khai Blade search UI


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P41,P31`
- **Phụ thuộc:** `T205`
- **File queue:** `tasks/T206_IMPLEMENT_BLADE_SEARCH_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai Blade search UI phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Accessible form/results/filter/pagination/empty/no-result.
2. escaped highlight.
3. locale routes/canonical.
...
- Hoàn thành đúng trọng tâm: Accessible form/results/filter/pagination/empty/no-result.
- Hoàn thành đúng trọng tâm: escaped highlight.
- Hoàn thành đúng trọng tâm: locale routes/canonical.
...
1. Cập nhật task `T206` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T206` trong message và chỉ stage file thuộc task.
