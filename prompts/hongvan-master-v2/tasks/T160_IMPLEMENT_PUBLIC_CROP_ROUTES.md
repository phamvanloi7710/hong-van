# T160 — Triển khai public query và routes Crop


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P34,P31`
- **Phụ thuộc:** `T159`
- **File queue:** `tasks/T160_IMPLEMENT_PUBLIC_CROP_ROUTES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai public query và routes Crop phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Published listing/detail/category/crop/stage.
2. locale/canonical/cache/empty states.
...
- Hoàn thành đúng trọng tâm: Published listing/detail/category/crop/stage.
- Hoàn thành đúng trọng tâm: locale/canonical/cache/empty states.
...
1. Cập nhật task `T160` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T160` trong message và chỉ stage file thuộc task.
