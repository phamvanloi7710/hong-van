# T118 — Làm cứng rollback bằng revision mới
...
- **Phụ thuộc:** `T117`
- **File queue:** `tasks/T118_HARDEN_ROLLBACK.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng rollback bằng revision mới phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Validate/migrate source version.
2. new revision with rollback_from/note.
3. pointer/audit/schedule/cache synchronization.
...
- Hoàn thành đúng trọng tâm: Validate/migrate source version.
- Hoàn thành đúng trọng tâm: new revision with rollback_from/note.
- Hoàn thành đúng trọng tâm: pointer/audit/schedule/cache synchronization.
...
1. Cập nhật task `T118` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T118` trong message và chỉ stage file thuộc task.
