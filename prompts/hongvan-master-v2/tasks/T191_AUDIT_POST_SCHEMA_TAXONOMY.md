# T191 — Rà schema Posts, categories và tags
...
- **Phụ thuộc:** `T190`
- **File queue:** `tasks/T191_AUDIT_POST_SCHEMA_TAXONOMY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema Posts, categories và tags phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Translations/slugs/author/media/status/schedule/tag links.
2. comments/index/unique/history.
...
- Hoàn thành đúng trọng tâm: Translations/slugs/author/media/status/schedule/tag links.
- Hoàn thành đúng trọng tâm: comments/index/unique/history.
...
1. Cập nhật task `T191` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T191` trong message và chỉ stage file thuộc task.
