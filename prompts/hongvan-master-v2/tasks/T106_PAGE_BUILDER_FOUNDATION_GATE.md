# T106 — Cổng Page Builder foundation P21-P27


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P21-P27`
- **Phụ thuộc:** `T105`
- **File queue:** `tasks/T106_PAGE_BUILDER_FOUNDATION_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng Page Builder foundation P21-P27 phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Http/Controllers/Api/V1/PageBuilder/`
- `BackEnd/resources/views/components/page-builder/`
...
```powershell
rg -n "BlockRegistry|PageDocument|Renderer|Sanitizer|PreviewSession|autosave|history" BackEnd Admin docs scripts docker 2>$null
```
...
1. Full PB backend tests, Angular tests/build, Playwright preview và security assertions pass.
2. không regress blocks/forms/media.
...
- Database không lưu Blade/PHP/JavaScript/CSS tùy ý.
- Editor không làm mất document local khi API trả conflict.
- Hoàn thành đúng trọng tâm: Full PB backend tests, Angular tests/build, Playwright preview và security assertions pass.
- Hoàn thành đúng trọng tâm: không regress blocks/forms/media.
...
1. Cập nhật task `T106` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T106` trong message và chỉ stage file thuộc task.
