# T094 — Tái kiểm định DataSourceRegistry
...
- **Phụ thuộc:** `T093`
- **File queue:** `tasks/T094_RECERTIFY_DYNAMIC_DATA_SOURCES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái kiểm định DataSourceRegistry phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Allowlisted source/filter/sort/limit/preset.
2. published-only/locale fallback.
3. không raw SQL/column.
...
- Hoàn thành đúng trọng tâm: Allowlisted source/filter/sort/limit/preset.
- Hoàn thành đúng trọng tâm: published-only/locale fallback.
- Hoàn thành đúng trọng tâm: không raw SQL/column.
...
1. Cập nhật task `T094` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T094` trong message và chỉ stage file thuộc task.
