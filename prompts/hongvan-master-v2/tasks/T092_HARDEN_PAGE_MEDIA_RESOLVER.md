# T092 — Làm cứng Media resolver cho Page Builder


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P23`
- **Phụ thuộc:** `T091`
- **File queue:** `tasks/T092_HARDEN_PAGE_MEDIA_RESOLVER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng Media resolver cho Page Builder phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Batch eager load fixed query count.
2. public-ready variants.
3. alt/decorative/dimensions/lazy/privacy video.
...
- Hoàn thành đúng trọng tâm: Batch eager load fixed query count.
- Hoàn thành đúng trọng tâm: public-ready variants.
- Hoàn thành đúng trọng tâm: alt/decorative/dimensions/lazy/privacy video.
...
1. Cập nhật task `T092` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T092` trong message và chỉ stage file thuộc task.
