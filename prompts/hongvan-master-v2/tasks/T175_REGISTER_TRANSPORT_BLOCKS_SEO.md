# T175 — Đăng ký Transportation blocks và SEO


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P24,P36,P42`
- **Phụ thuộc:** `T174`
- **File queue:** `tasks/T175_REGISTER_TRANSPORT_BLOCKS_SEO.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Đăng ký Transportation blocks và SEO phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Fleet/route/capability/form blocks.
2. meta/schema/breadcrumb/cache.
...
- Hoàn thành đúng trọng tâm: Fleet/route/capability/form blocks.
- Hoàn thành đúng trọng tâm: meta/schema/breadcrumb/cache.
...
1. Cập nhật task `T175` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T175` trong message và chỉ stage file thuộc task.
