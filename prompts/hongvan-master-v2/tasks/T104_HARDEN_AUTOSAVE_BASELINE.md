# T104 — Làm cứng orchestration autosave cơ bản
...
- **Phụ thuộc:** `T103`
- **File queue:** `tasks/T104_HARDEN_AUTOSAVE_BASELINE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng orchestration autosave cơ bản phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Debounce/coalesce/serialize request.
2. manual save waits.
...
- Hoàn thành đúng trọng tâm: Debounce/coalesce/serialize request.
- Hoàn thành đúng trọng tâm: manual save waits.
- Hoàn thành đúng trọng tâm: cleanup subscriptions/timers.
- Hoàn thành đúng trọng tâm: dirty navigation guard.
...
1. Cập nhật task `T104` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T104` trong message và chỉ stage file thuộc task.
