# T136 — Triển khai resolver localized slug
...
- **Phụ thuộc:** `T135`
- **File queue:** `tasks/T136_IMPLEMENT_LOCALIZED_SLUG_RESOLVER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai resolver localized slug phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Namespace/type/locale lookup.
2. fallback/old slug redirect.
3. published-only và query efficient.
...
- Hoàn thành đúng trọng tâm: Namespace/type/locale lookup.
- Hoàn thành đúng trọng tâm: fallback/old slug redirect.
- Hoàn thành đúng trọng tâm: published-only và query efficient.
...
1. Cập nhật task `T136` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T136` trong message và chỉ stage file thuộc task.
