# T195 — Triển khai public post listing, category và tag


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P39,P31`
- **Phụ thuộc:** `T194`
- **File queue:** `tasks/T195_IMPLEMENT_PUBLIC_POST_LISTINGS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai public post listing, category và tag phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Published locale routes/pagination/canonical/empty states.
...
- Hoàn thành đúng trọng tâm: Published locale routes/pagination/canonical/empty states.
- Hoàn thành đúng trọng tâm: no duplicate content.
...
1. Cập nhật task `T195` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T195` trong message và chỉ stage file thuộc task.
