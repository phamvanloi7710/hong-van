# T078 — Ổn định draft, preview, publish và rollback Theme


## Metadata

- **Giai đoạn:** E — Public Blade foundation và Theme Studio
- **Bao phủ prompt gốc:** `P20`
- **Phụ thuộc:** `T077`
- **File queue:** `tasks/T078_STABILIZE_THEME_LIFECYCLE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định draft, preview, publish và rollback Theme phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/Themes/`
- `BackEnd/app/Models/Theme*.php`
- `BackEnd/config/public_theme.php`
- `Admin/src/app/features/theme-studio/`
...
```powershell
rg -n "ThemeTokenSchema|ThemeCssCompiler|preview|publish|rollback|token" BackEnd Admin docs scripts docker 2>$null
```
...
1. Signed expiring noindex preview.
2. transaction/audit/cache.
3. rollback version mới hoặc pointer contract bất biến.
...
- Không cho CSS/JavaScript tùy ý từ database.
- Preview và published dùng cùng compiler phía server.
- Published theme version bất biến và rollback có audit/cache invalidation.
- Hoàn thành đúng trọng tâm: Signed expiring noindex preview.
- Hoàn thành đúng trọng tâm: transaction/audit/cache.
- Hoàn thành đúng trọng tâm: rollback version mới hoặc pointer contract bất biến.
...
cd Admin; npm test -- --run theme-studio
...
1. Cập nhật task `T078` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T078` trong message và chỉ stage file thuộc task.
