# T095 — Làm cứng dynamic loader, memoization và cache


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P24`
- **Phụ thuộc:** `T094`
- **File queue:** `tasks/T095_HARDEN_DYNAMIC_LOADER_CACHE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng dynamic loader, memoization và cache phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Duplicate binding request memoization.
2. no N+1.
3. empty/error state an toàn.
4. dependency tags.
...
- Hoàn thành đúng trọng tâm: Duplicate binding request memoization.
- Hoàn thành đúng trọng tâm: no N+1.
- Hoàn thành đúng trọng tâm: empty/error state an toàn.
- Hoàn thành đúng trọng tâm: dependency tags.
...
1. Cập nhật task `T095` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T095` trong message và chỉ stage file thuộc task.
