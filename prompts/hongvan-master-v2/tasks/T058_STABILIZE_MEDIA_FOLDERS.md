...
- **Phụ thuộc:** `T057`
- **File queue:** `tasks/T058_STABILIZE_MEDIA_FOLDERS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định thư mục Media phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Create/rename/move/lock.
2. chặn cycle và move vào descendant.
3. root semantics rõ.
...
- Hoàn thành đúng trọng tâm: Create/rename/move/lock.
- Hoàn thành đúng trọng tâm: chặn cycle và move vào descendant.
- Hoàn thành đúng trọng tâm: root semantics rõ.
...
1. Cập nhật task `T058` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T058` trong message và chỉ stage file thuộc task.
