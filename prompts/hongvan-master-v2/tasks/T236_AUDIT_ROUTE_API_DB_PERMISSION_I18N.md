# T236 — Audit chéo route, API, DB, permission và i18n
...
- **Phụ thuộc:** `T235`
- **File queue:** `tasks/T236_AUDIT_ROUTE_API_DB_PERMISSION_I18N.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Audit chéo route, API, DB, permission và i18n phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Find orphan/missing routes/controllers/models/tables/permissions/translations.
2. validate public/private boundaries.
3. detect dead config.
...
- Hoàn thành đúng trọng tâm: Find orphan/missing routes/controllers/models/tables/permissions/translations.
- Hoàn thành đúng trọng tâm: validate public/private boundaries.
- Hoàn thành đúng trọng tâm: detect dead config.
...
1. Cập nhật task `T236` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T236` trong message và chỉ stage file thuộc task.
