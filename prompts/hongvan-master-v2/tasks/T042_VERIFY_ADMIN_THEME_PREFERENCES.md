# T042 — Xác minh theme preferences theo user


## Metadata

- **Giai đoạn:** C — Authentication, RBAC, Preferences, Settings, Localization và Audit
- **Bao phủ prompt gốc:** `P12`
- **Phụ thuộc:** `T041`
- **File queue:** `tasks/T042_VERIFY_ADMIN_THEME_PREFERENCES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Xác minh theme preferences theo user phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/Identity/UserPreferenceService.php`
- `BackEnd/config/admin_preferences.php`
- `Admin/src/app/core/preferences/`
- `Admin/src/app/core/theme/`
...
```powershell
rg -n "preferences|favorites|locale|theme|namespace" BackEnd Admin docs scripts docker 2>$null
```
...
1. Theme server persistence và cache local.
2. không flash sai theme.
3. responsive/theme changes được restore sau login.
...
- Preference contract có allowlist và schema typed.
- Server là nguồn chân lý, local cache chỉ giảm flash.
- Favorite menu giữ đúng cơ chế chọn nhiều và thứ tự Annular.
- Hoàn thành đúng trọng tâm: Theme server persistence và cache local.
- Hoàn thành đúng trọng tâm: không flash sai theme.
- Hoàn thành đúng trọng tâm: responsive/theme changes được restore sau login.
...
```text
cd BackEnd; php artisan test --filter=Preference
cd Admin; npm test -- --run preferences
...
1. Cập nhật task `T042` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T042` trong message và chỉ stage file thuộc task.
