# T081 — Rà schema database Page Builder
...
- **Phụ thuộc:** `T080`
- **File queue:** `tasks/T081_AUDIT_PAGE_BUILDER_SCHEMA.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema database Page Builder phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Pages/translations/versions/schedules/locks/templates/preview tables.
2. comments/FK/pointers/immutability.
...
- Hoàn thành đúng trọng tâm: Pages/translations/versions/schedules/locks/templates/preview tables.
- Hoàn thành đúng trọng tâm: comments/FK/pointers/immutability.
...
1. Cập nhật task `T081` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T081` trong message và chỉ stage file thuộc task.
