...
- **Phụ thuộc:** `T199`
- **File queue:** `tasks/T200_STABILIZE_SHOWCASE_ADMIN_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định Angular Showcase phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Four tabs/dialogs/media picker/PDF visibility.
2. typed/i18n/permission/responsive.
...
- Hoàn thành đúng trọng tâm: Four tabs/dialogs/media picker/PDF visibility.
- Hoàn thành đúng trọng tâm: typed/i18n/permission/responsive.
...
1. Cập nhật task `T200` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T200` trong message và chỉ stage file thuộc task.
