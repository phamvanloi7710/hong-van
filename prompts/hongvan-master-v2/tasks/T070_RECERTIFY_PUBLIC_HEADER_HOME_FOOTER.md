# T070 — Tái kiểm định header, home và footer public
...
- **Phụ thuộc:** `T069`
- **File queue:** `tasks/T070_RECERTIFY_PUBLIC_HEADER_HOME_FOOTER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái kiểm định header, home và footer public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Parity desktop/tablet/mobile.
2. menu accessible.
3. content doanh nghiệp không dùng dữ liệu mẫu sai.
...
- Hoàn thành đúng trọng tâm: Parity desktop/tablet/mobile.
- Hoàn thành đúng trọng tâm: menu accessible.
- Hoàn thành đúng trọng tâm: content doanh nghiệp không dùng dữ liệu mẫu sai.
...
1. Cập nhật task `T070` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T070` trong message và chỉ stage file thuộc task.
