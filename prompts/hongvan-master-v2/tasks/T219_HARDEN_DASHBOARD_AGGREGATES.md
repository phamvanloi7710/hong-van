# T219 — Làm cứng aggregate và visibility Dashboard
...
- **Phụ thuộc:** `T218`
- **File queue:** `tasks/T219_HARDEN_DASHBOARD_AGGREGATES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng aggregate và visibility Dashboard phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Permission/assigned lead scoping.
2. date ranges/timezone/cache.
3. no cross-tenant/user leak.
...
- Hoàn thành đúng trọng tâm: Permission/assigned lead scoping.
- Hoàn thành đúng trọng tâm: date ranges/timezone/cache.
- Hoàn thành đúng trọng tâm: no cross-tenant/user leak.
...
1. Cập nhật task `T219` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T219` trong message và chỉ stage file thuộc task.
