# T059 — Ổn định metadata, visibility và lock Media
...
- **Phụ thuộc:** `T058`
- **File queue:** `tasks/T059_STABILIZE_MEDIA_METADATA_VISIBILITY_LOCK.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định metadata, visibility và lock Media phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Alt/title/caption localized nếu contract.
2. private/public.
3. locked record trả 409 cho mutation.
...
- Hoàn thành đúng trọng tâm: Alt/title/caption localized nếu contract.
- Hoàn thành đúng trọng tâm: private/public.
- Hoàn thành đúng trọng tâm: locked record trả 409 cho mutation.
...
1. Cập nhật task `T059` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T059` trong message và chỉ stage file thuộc task.
