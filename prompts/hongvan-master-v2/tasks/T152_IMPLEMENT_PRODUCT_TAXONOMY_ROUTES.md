# T152 — Triển khai route category, brand và tag public


## Metadata

- **Giai đoạn:** I — Product Catalog hoàn chỉnh
- **Bao phủ prompt gốc:** `P33,P31`
- **Phụ thuộc:** `T151`
- **File queue:** `tasks/T152_IMPLEMENT_PRODUCT_TAXONOMY_ROUTES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai route category, brand và tag public phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Localized slugs/canonical/pagination/empty state.
...
- Hoàn thành đúng trọng tâm: Localized slugs/canonical/pagination/empty state.
- Hoàn thành đúng trọng tâm: no duplicate content.
- Hoàn thành đúng trọng tâm: internal links.
...
1. Cập nhật task `T152` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T152` trong message và chỉ stage file thuộc task.
