# T005 — Đối chiếu prompt gốc, commit và task ledger


## Metadata

- **Giai đoạn:** A — Quản trị, baseline và bộ nhớ dự án
- **Bao phủ prompt gốc:** `P00-P56`
- **Phụ thuộc:** `T004`
- **File queue:** `tasks/T005_RECONCILE_PROMPTS_LEDGER.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Đối chiếu prompt gốc, commit và task ledger phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Lập bảng P00-P56 theo IMPLEMENTED/PARTIAL/MISSING/STALE/BLOCKED.
2. đính kèm commit/file/test evidence.
3. không đánh dấu DONE chỉ từ mô tả cũ.
...
- Hoàn thành đúng trọng tâm: Lập bảng P00-P56 theo IMPLEMENTED/PARTIAL/MISSING/STALE/BLOCKED.
- Hoàn thành đúng trọng tâm: đính kèm commit/file/test evidence.
- Hoàn thành đúng trọng tâm: không đánh dấu DONE chỉ từ mô tả cũ.
...
1. Cập nhật task `T005` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T005` trong message và chỉ stage file thuộc task.
