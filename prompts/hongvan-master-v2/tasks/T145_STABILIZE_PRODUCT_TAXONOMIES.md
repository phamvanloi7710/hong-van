# T145 — Ổn định category, brand, tag và attribute


## Metadata

- **Giai đoạn:** I — Product Catalog hoàn chỉnh
- **Bao phủ prompt gốc:** `P32,P33`
- **Phụ thuộc:** `T144`
- **File queue:** `tasks/T145_STABILIZE_PRODUCT_TAXONOMIES.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định category, brand, tag và attribute phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Localized hierarchy/slugs/order/status.
2. cycle/duplicate constraints.
3. attribute typed values.
...
- Hoàn thành đúng trọng tâm: Localized hierarchy/slugs/order/status.
- Hoàn thành đúng trọng tâm: cycle/duplicate constraints.
- Hoàn thành đúng trọng tâm: attribute typed values.
...
1. Cập nhật task `T145` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T145` trong message và chỉ stage file thuộc task.
