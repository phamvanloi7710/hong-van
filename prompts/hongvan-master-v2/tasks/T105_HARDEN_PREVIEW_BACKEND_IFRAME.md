# T105 — Làm cứng preview session và iframe protocol


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P27`
- **Phụ thuộc:** `T104`
- **File queue:** `tasks/T105_HARDEN_PREVIEW_BACKEND_IFRAME.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng preview session và iframe protocol phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Owner-scoped cache TTL, hashed token, signed URL, noindex/CSP.
2. postMessage exact origin/source/token/schema.
...
- Hoàn thành đúng trọng tâm: Owner-scoped cache TTL, hashed token, signed URL, noindex/CSP.
- Hoàn thành đúng trọng tâm: postMessage exact origin/source/token/schema.
...
1. Cập nhật task `T105` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T105` trong message và chỉ stage file thuộc task.
