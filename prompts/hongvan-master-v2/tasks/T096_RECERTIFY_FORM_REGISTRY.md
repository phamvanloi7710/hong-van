# T096 — Tái kiểm định registry Form blocks


## Metadata

- **Giai đoạn:** F — Page Builder foundation, blocks, editor và preview
- **Bao phủ prompt gốc:** `P25`
- **Phụ thuộc:** `T095`
- **File queue:** `tasks/T096_RECERTIFY_FORM_REGISTRY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Tái kiểm định registry Form blocks phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Contact/product quote/transport/warehouse versioned contracts.
2. field/action/validation/layout allowlist.
...
- Hoàn thành đúng trọng tâm: Contact/product quote/transport/warehouse versioned contracts.
- Hoàn thành đúng trọng tâm: field/action/validation/layout allowlist.
...
1. Cập nhật task `T096` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T096` trong message và chỉ stage file thuộc task.
