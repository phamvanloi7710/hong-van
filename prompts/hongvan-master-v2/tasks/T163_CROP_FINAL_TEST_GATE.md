# T163 — Cổng test/E2E Crop Solutions
...
- **Phụ thuộc:** `T162`
- **File queue:** `tasks/T163_CROP_FINAL_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng test/E2E Crop Solutions phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Hierarchy/cycle/CRUD/public/timeline/PB/SEO/locale/mobile tests pass.
...
- Hoàn thành đúng trọng tâm: Hierarchy/cycle/CRUD/public/timeline/PB/SEO/locale/mobile tests pass.
...
1. Cập nhật task `T163` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T163` trong message và chỉ stage file thuộc task.
