# T186 — Làm cứng intake, idempotency và dedup
...
- **Phụ thuộc:** `T185`
- **File queue:** `tasks/T186_HARDEN_LEAD_INTAKE_DEDUP.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng intake, idempotency và dedup phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Four public flows.
2. honeypot/consent/rate/idempotency window.
...
- Hoàn thành đúng trọng tâm: Four public flows.
- Hoàn thành đúng trọng tâm: honeypot/consent/rate/idempotency window.
- Hoàn thành đúng trọng tâm: no duplicate notifications.
...
1. Cập nhật task `T186` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T186` trong message và chỉ stage file thuộc task.
