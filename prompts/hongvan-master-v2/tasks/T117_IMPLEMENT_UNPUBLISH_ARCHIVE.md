# T117 — Hoàn thiện unpublish và archive page
...
- **Phụ thuộc:** `T116`
- **File queue:** `tasks/T117_IMPLEMENT_UNPUBLISH_ARCHIVE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Hoàn thiện unpublish và archive page phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Không xóa lịch sử.
2. pointer/status transaction.
3. cache/sitemap after commit.
4. public content biến mất đúng.
...
- Hoàn thành đúng trọng tâm: Không xóa lịch sử.
- Hoàn thành đúng trọng tâm: pointer/status transaction.
- Hoàn thành đúng trọng tâm: cache/sitemap after commit.
- Hoàn thành đúng trọng tâm: public content biến mất đúng.
...
1. Cập nhật task `T117` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T117` trong message và chỉ stage file thuộc task.
