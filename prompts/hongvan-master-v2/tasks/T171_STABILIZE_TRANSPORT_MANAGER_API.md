# T171 — Ổn định API fleet, routes và areas
...
- **Phụ thuộc:** `T170`
- **File queue:** `tasks/T171_STABILIZE_TRANSPORT_MANAGER_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định API fleet, routes và areas phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CRUD/publish/featured/order.
2. policy/audit/cache/query.
3. media usage.
...
- Hoàn thành đúng trọng tâm: CRUD/publish/featured/order.
- Hoàn thành đúng trọng tâm: policy/audit/cache/query.
- Hoàn thành đúng trọng tâm: media usage.
...
1. Cập nhật task `T171` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T171` trong message và chỉ stage file thuộc task.
