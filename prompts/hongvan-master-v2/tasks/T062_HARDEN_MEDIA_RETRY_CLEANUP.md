# T062 — Làm cứng retry và cleanup Media
...
- **Phụ thuộc:** `T061`
- **File queue:** `tasks/T062_HARDEN_MEDIA_RETRY_CLEANUP.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng retry và cleanup Media phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Command có dry-run/batch/lock.
2. không xóa file đang xử lý.
3. log không chứa nội dung file.
...
- Hoàn thành đúng trọng tâm: Command có dry-run/batch/lock.
- Hoàn thành đúng trọng tâm: không xóa file đang xử lý.
- Hoàn thành đúng trọng tâm: log không chứa nội dung file.
...
1. Cập nhật task `T062` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T062` trong message và chỉ stage file thuộc task.
