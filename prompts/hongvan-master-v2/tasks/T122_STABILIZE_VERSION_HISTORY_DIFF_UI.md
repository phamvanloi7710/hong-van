# T122 — Ổn định history và diff UI
...
- **Phụ thuộc:** `T121`
- **File queue:** `tasks/T122_STABILIZE_VERSION_HISTORY_DIFF_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định history và diff UI phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Version/status/author/time/note/checksum/live/scheduled.
2. preview/compare/publish/rollback actions permission-aware.
...
- Hoàn thành đúng trọng tâm: Version/status/author/time/note/checksum/live/scheduled.
- Hoàn thành đúng trọng tâm: preview/compare/publish/rollback actions permission-aware.
...
1. Cập nhật task `T122` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T122` trong message và chỉ stage file thuộc task.
