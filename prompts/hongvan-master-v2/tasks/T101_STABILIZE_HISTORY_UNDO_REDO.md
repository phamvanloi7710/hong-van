# T101 — Ổn định history, undo và redo
...
- **Phụ thuộc:** `T100`
- **File queue:** `tasks/T101_STABILIZE_HISTORY_UNDO_REDO.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định history, undo và redo phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Bounded history.
2. dirty/saved markers.
3. không mất selection hoặc checksum state.
...
- Hoàn thành đúng trọng tâm: Bounded history.
- Hoàn thành đúng trọng tâm: dirty/saved markers.
- Hoàn thành đúng trọng tâm: không mất selection hoặc checksum state.
...
1. Cập nhật task `T101` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T101` trong message và chỉ stage file thuộc task.
