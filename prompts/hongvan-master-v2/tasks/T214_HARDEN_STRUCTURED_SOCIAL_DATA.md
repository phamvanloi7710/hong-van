# T214 — Làm cứng structured data và social cards
...
- **Phụ thuộc:** `T213`
- **File queue:** `tasks/T214_HARDEN_STRUCTURED_SOCIAL_DATA.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng structured data và social cards phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Organization/Breadcrumb/Product/Article/FAQ schemas from real data.
...
- Hoàn thành đúng trọng tâm: Organization/Breadcrumb/Product/Article/FAQ schemas from real data.
- Hoàn thành đúng trọng tâm: OG/Twitter fallbacks.
- Hoàn thành đúng trọng tâm: JSON safe.
...
1. Cập nhật task `T214` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T214` trong message và chỉ stage file thuộc task.
