# T168 — Đăng ký Service blocks và SEO


## Metadata

- **Giai đoạn:** J — Crop Solutions, Services, Transportation và Warehouses
- **Bao phủ prompt gốc:** `P24,P35,P42`
- **Phụ thuộc:** `T167`
- **File queue:** `tasks/T168_REGISTER_SERVICE_BLOCKS_SEO.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Đăng ký Service blocks và SEO phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Grid/detail CTA blocks.
2. meta/schema/breadcrumb/cache dependencies.
...
- Hoàn thành đúng trọng tâm: Grid/detail CTA blocks.
- Hoàn thành đúng trọng tâm: meta/schema/breadcrumb/cache dependencies.
...
1. Cập nhật task `T168` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T168` trong message và chỉ stage file thuộc task.
