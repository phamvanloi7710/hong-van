# T085 — Làm cứng block/document migrator
...
- **Phụ thuộc:** `T084`
- **File queue:** `tasks/T085_HARDEN_BLOCK_MIGRATOR.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng block/document migrator phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Chỉ migration tuần tự.
2. future/missing migration bị từ chối.
3. round-trip checksum/test fixture.
...
- Hoàn thành đúng trọng tâm: Chỉ migration tuần tự.
- Hoàn thành đúng trọng tâm: future/missing migration bị từ chối.
- Hoàn thành đúng trọng tâm: round-trip checksum/test fixture.
...
1. Cập nhật task `T085` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T085` trong message và chỉ stage file thuộc task.
