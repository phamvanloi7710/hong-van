# T153 — Triển khai detail, giá và CTA báo giá
...
- **Phụ thuộc:** `T152`
- **File queue:** `tasks/T153_IMPLEMENT_PRODUCT_DETAIL_PRICE_QUOTE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai detail, giá và CTA báo giá phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Semantic detail/spec/media/related.
2. price contract.
3. contact CTA/signed product context.
4. no cart.
...
- Hoàn thành đúng trọng tâm: Semantic detail/spec/media/related.
- Hoàn thành đúng trọng tâm: price contract.
- Hoàn thành đúng trọng tâm: contact CTA/signed product context.
- Hoàn thành đúng trọng tâm: no cart.
...
1. Cập nhật task `T153` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T153` trong message và chỉ stage file thuộc task.
