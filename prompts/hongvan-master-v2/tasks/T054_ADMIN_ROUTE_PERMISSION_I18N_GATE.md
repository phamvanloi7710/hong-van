# T054 — Cổng route, permission và i18n của Admin


## Metadata

- **Giai đoạn:** D — Admin shell và Media Manager
- **Bao phủ prompt gốc:** `P06,P11,P12`
- **Phụ thuộc:** `T053`
- **File queue:** `tasks/T054_ADMIN_ROUTE_PERMISSION_I18N_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng route, permission và i18n của Admin phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Mọi leaf menu clickable hoặc ẩn hợp lệ.
2. route guard đúng permission.
3. không còn hardcoded user text.
...
- Hoàn thành đúng trọng tâm: Mọi leaf menu clickable hoặc ẩn hợp lệ.
- Hoàn thành đúng trọng tâm: route guard đúng permission.
- Hoàn thành đúng trọng tâm: không còn hardcoded user text.
...
1. Cập nhật task `T054` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T054` trong message và chỉ stage file thuộc task.
