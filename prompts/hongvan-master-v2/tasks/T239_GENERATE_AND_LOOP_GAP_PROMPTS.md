# T239 — Sinh prompt focused cho mọi gap và lặp audit
...
- **Phụ thuộc:** `T238`
- **File queue:** `tasks/T239_GENERATE_AND_LOOP_GAP_PROMPTS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Sinh prompt focused cho mọi gap và lặp audit phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Mỗi independent gap tạo generated/Gxxx_slug.md + queue.
2. run one by one.
3. after queue empty rerun T235-T239 until zero gaps.
4. không mega-fix.
...
- Hoàn thành đúng trọng tâm: Mỗi independent gap tạo generated/Gxxx_slug.md + queue.
- Hoàn thành đúng trọng tâm: run one by one.
- Hoàn thành đúng trọng tâm: after queue empty rerun T235-T239 until zero gaps.
- Hoàn thành đúng trọng tâm: không mega-fix.
...
1. Cập nhật task `T239` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T239` trong message và chỉ stage file thuộc task.
