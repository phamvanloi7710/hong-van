# T205 — Làm cứng API search và filters


## Metadata

- **Giai đoạn:** K — Leads, News, Showcase và Search
- **Bao phủ prompt gốc:** `P41`
- **Phụ thuộc:** `T204`
- **File queue:** `tasks/T205_HARDEN_PUBLIC_SEARCH_API.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng API search và filters phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
- `BackEnd/app/Domain/Search/`
- `BackEnd/app/Http/Controllers/Api/V1/Search/`
- `BackEnd/config/search.php`
...
```powershell
rg -n "FULLTEXT|accent|normalize|highlight|related|search_logs" BackEnd Admin docs scripts docker 2>$null
```
...
1. Published-only entity types.
2. allowlisted filters/pagination/sorts.
3. bound Boolean FULLTEXT.
4. rate limit.
...
- Query được bind/allowlist, không raw SQL từ request.
- Highlight được escape và không tạo XSS.
- Search chỉ trả entity published/active locale và log tuân thủ privacy.
- Hoàn thành đúng trọng tâm: Published-only entity types.
- Hoàn thành đúng trọng tâm: allowlisted filters/pagination/sorts.
- Hoàn thành đúng trọng tâm: bound Boolean FULLTEXT.
- Hoàn thành đúng trọng tâm: rate limit.
...
cd BackEnd; php artisan search:reindex --help
```
...
1. Cập nhật task `T205` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T205` trong message và chỉ stage file thuộc task.
