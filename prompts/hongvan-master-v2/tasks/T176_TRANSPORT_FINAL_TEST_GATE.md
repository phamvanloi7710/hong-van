# T176 — Cổng test/E2E Transportation
...
- **Phụ thuộc:** `T175`
- **File queue:** `tasks/T176_TRANSPORT_FINAL_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng test/E2E Transportation phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Admin/public/request/lead/PB/SEO/locale/mobile/security tests pass.
...
- Hoàn thành đúng trọng tâm: Admin/public/request/lead/PB/SEO/locale/mobile/security tests pass.
...
1. Cập nhật task `T176` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T176` trong message và chỉ stage file thuộc task.
