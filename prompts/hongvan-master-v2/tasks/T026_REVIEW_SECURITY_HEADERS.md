# T026 — Rà security headers, trusted host và proxy


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P15`
- **Phụ thuộc:** `T025`
- **File queue:** `tasks/T026_REVIEW_SECURITY_HEADERS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà security headers, trusted host và proxy phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. CSP/frame/nosniff/referrer/HSTS đúng context.
2. preview same-origin riêng.
3. trusted host/proxy lấy environment an toàn.
...
- Hoàn thành đúng trọng tâm: CSP/frame/nosniff/referrer/HSTS đúng context.
- Hoàn thành đúng trọng tâm: preview same-origin riêng.
- Hoàn thành đúng trọng tâm: trusted host/proxy lấy environment an toàn.
...
1. Cập nhật task `T026` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T026` trong message và chỉ stage file thuộc task.
