# T141 — Làm cứng cache invalidation Menu và Regions
...
- **Phụ thuộc:** `T140`
- **File queue:** `tasks/T141_HARDEN_MENU_REGION_CACHE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng cache invalidation Menu và Regions phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Locale/version/theme dependencies.
2. afterCommit invalidation.
3. no stale header/footer after publish.
...
- Hoàn thành đúng trọng tâm: Locale/version/theme dependencies.
- Hoàn thành đúng trọng tâm: afterCommit invalidation.
- Hoàn thành đúng trọng tâm: no stale header/footer after publish.
...
1. Cập nhật task `T141` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T141` trong message và chỉ stage file thuộc task.
