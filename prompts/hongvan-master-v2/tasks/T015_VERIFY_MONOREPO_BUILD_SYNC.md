# T015 — Ổn định build Angular và đồng bộ Laravel


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P07`
- **Phụ thuộc:** `T014`
- **File queue:** `tasks/T015_VERIFY_MONOREPO_BUILD_SYNC.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Ổn định build Angular và đồng bộ Laravel phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Kiểm tra angular output path và sync-to-laravel.
2. không chỉnh tay output.
3. build production phải đồng bộ BackEnd/public/admin/browser xác định.
...
- Hoàn thành đúng trọng tâm: Kiểm tra angular output path và sync-to-laravel.
- Hoàn thành đúng trọng tâm: không chỉnh tay output.
- Hoàn thành đúng trọng tâm: build production phải đồng bộ BackEnd/public/admin/browser xác định.
...
1. Cập nhật task `T015` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T015` trong message và chỉ stage file thuộc task.
