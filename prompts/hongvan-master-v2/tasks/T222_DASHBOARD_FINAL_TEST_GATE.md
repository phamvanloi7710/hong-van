...
- **Phụ thuộc:** `T221`
- **File queue:** `tasks/T222_DASHBOARD_FINAL_TEST_GATE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Cổng test/E2E Dashboard phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Visibility/metrics/timezone/notifications/reports/private downloads/Admin UI/console/mobile pass.
...
- Hoàn thành đúng trọng tâm: Visibility/metrics/timezone/notifications/reports/private downloads/Admin UI/console/mobile pass.
...
1. Cập nhật task `T222` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T222` trong message và chỉ stage file thuộc task.
