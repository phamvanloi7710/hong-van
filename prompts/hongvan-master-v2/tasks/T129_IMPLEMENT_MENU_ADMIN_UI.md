# T129 — Triển khai Angular quản lý Menu
...
- **Phụ thuộc:** `T128`
- **File queue:** `tasks/T129_IMPLEMENT_MENU_ADMIN_UI.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai Angular quản lý Menu phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Tree drag/drop, localized labels/links, status/order.
2. dirty guard/dialogs/permission/i18n/build.
...
- Hoàn thành đúng trọng tâm: Tree drag/drop, localized labels/links, status/order.
- Hoàn thành đúng trọng tâm: dirty guard/dialogs/permission/i18n/build.
...
1. Cập nhật task `T129` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T129` trong message và chỉ stage file thuộc task.
