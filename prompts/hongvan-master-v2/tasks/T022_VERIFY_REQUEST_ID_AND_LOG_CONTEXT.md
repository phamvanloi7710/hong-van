# T022 — Xác minh request ID và log context


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P09,P15`
- **Phụ thuộc:** `T021`
- **File queue:** `tasks/T022_VERIFY_REQUEST_ID_AND_LOG_CONTEXT.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Xác minh request ID và log context phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Chỉ chấp nhận ULID hợp lệ hoặc tạo mới.
2. header và meta phải trùng.
3. không log body/token/cookie/credential.
...
- Hoàn thành đúng trọng tâm: Chỉ chấp nhận ULID hợp lệ hoặc tạo mới.
- Hoàn thành đúng trọng tâm: header và meta phải trùng.
- Hoàn thành đúng trọng tâm: không log body/token/cookie/credential.
...
1. Cập nhật task `T022` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T022` trong message và chỉ stage file thuộc task.
