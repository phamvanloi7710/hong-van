# T170 — Rà schema và phạm vi Transportation
...
- **Phụ thuộc:** `T169`
- **File queue:** `tasks/T170_AUDIT_TRANSPORT_SCHEMA_BOUNDARY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema và phạm vi Transportation phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Vehicle types/fleet/media/routes/areas/requests/history.
2. không GPS/dispatch/auto fare.
...
- Hoàn thành đúng trọng tâm: Vehicle types/fleet/media/routes/areas/requests/history.
- Hoàn thành đúng trọng tâm: không GPS/dispatch/auto fare.
...
1. Cập nhật task `T170` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T170` trong message và chỉ stage file thuộc task.
