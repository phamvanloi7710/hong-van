# T150 — Làm cứng bulk, publish, archive, trash và restore


## Metadata

- **Giai đoạn:** I — Product Catalog hoàn chỉnh
- **Bao phủ prompt gốc:** `P33`
- **Phụ thuộc:** `T149`
- **File queue:** `tasks/T150_HARDEN_PRODUCT_BULK_LIFECYCLE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng bulk, publish, archive, trash và restore phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Per-resource authorization.
2. batch limits.
3. transaction/audit/cache.
4. published dependencies.
...
- Hoàn thành đúng trọng tâm: Per-resource authorization.
- Hoàn thành đúng trọng tâm: batch limits.
- Hoàn thành đúng trọng tâm: transaction/audit/cache.
- Hoàn thành đúng trọng tâm: published dependencies.
...
1. Cập nhật task `T150` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T150` trong message và chỉ stage file thuộc task.
