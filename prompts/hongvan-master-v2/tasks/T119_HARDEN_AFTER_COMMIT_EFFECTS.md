# T119 — Chuẩn hóa cache, sitemap và audit sau commit
...
- **Phụ thuộc:** `T118`
- **File queue:** `tasks/T119_HARDEN_AFTER_COMMIT_EFFECTS.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Chuẩn hóa cache, sitemap và audit sau commit phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Không invalidate/dispatch trước commit.
2. failed transaction giữ cache/pointer.
3. events idempotent và observable.
...
- Hoàn thành đúng trọng tâm: Không invalidate/dispatch trước commit.
- Hoàn thành đúng trọng tâm: failed transaction giữ cache/pointer.
- Hoàn thành đúng trọng tâm: events idempotent và observable.
...
1. Cập nhật task `T119` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T119` trong message và chỉ stage file thuộc task.
