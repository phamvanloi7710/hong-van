# T203 — Làm cứng normalization và FULLTEXT index
...
- **Phụ thuộc:** `T202`
- **File queue:** `tasks/T203_HARDEN_SEARCH_NORMALIZATION_INDEX.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng normalization và FULLTEXT index phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Vietnamese accent folding/canonical text.
2. index columns/migrations.
3. query term bounds.
...
- Hoàn thành đúng trọng tâm: Vietnamese accent folding/canonical text.
- Hoàn thành đúng trọng tâm: index columns/migrations.
- Hoàn thành đúng trọng tâm: query term bounds.
...
1. Cập nhật task `T203` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T203` trong message và chỉ stage file thuộc task.
