# T177 — Rà schema Warehouse, facilities và services
...
- **Phụ thuộc:** `T176`
- **File queue:** `tasks/T177_AUDIT_WAREHOUSE_SCHEMA.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Rà schema Warehouse, facilities và services phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Warehouses/translations/media/facilities/services/requests/history.
2. comments/FK/status/order.
...
- Hoàn thành đúng trọng tâm: Warehouses/translations/media/facilities/services/requests/history.
- Hoàn thành đúng trọng tâm: comments/FK/status/order.
...
1. Cập nhật task `T177` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T177` trong message và chỉ stage file thuộc task.
