# T207 — Làm cứng related content và search logs
...
- **Phụ thuộc:** `T206`
- **File queue:** `tasks/T207_HARDEN_RELATED_SEARCH_LOGS_PRIVACY.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng related content và search logs phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Type/public ID validation.
2. published-only related.
3. privacy-safe logs/retention/no raw PII.
...
- Hoàn thành đúng trọng tâm: Type/public ID validation.
- Hoàn thành đúng trọng tâm: published-only related.
- Hoàn thành đúng trọng tâm: privacy-safe logs/retention/no raw PII.
...
1. Cập nhật task `T207` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T207` trong message và chỉ stage file thuộc task.
