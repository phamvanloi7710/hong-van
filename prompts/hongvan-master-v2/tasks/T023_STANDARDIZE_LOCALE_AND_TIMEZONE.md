# T023 — Chuẩn hóa locale API và timezone


## Metadata

- **Giai đoạn:** B — Nền tảng Laravel, Angular, Database và API
- **Bao phủ prompt gốc:** `P09,P14`
- **Phụ thuộc:** `T022`
- **File queue:** `tasks/T023_STANDARDIZE_LOCALE_AND_TIMEZONE.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Chuẩn hóa locale API và timezone phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Thứ tự locale user/header/Accept-Language/default.
2. DB/app UTC.
3. API ISO8601 Z và Admin Asia/Ho_Chi_Minh.
...
- Hoàn thành đúng trọng tâm: Thứ tự locale user/header/Accept-Language/default.
- Hoàn thành đúng trọng tâm: DB/app UTC.
- Hoàn thành đúng trọng tâm: API ISO8601 Z và Admin Asia/Ho_Chi_Minh.
...
1. Cập nhật task `T023` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T023` trong message và chỉ stage file thuộc task.
