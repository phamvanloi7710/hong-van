# T154 — Triển khai SEO và structured data Product


## Metadata

- **Giai đoạn:** I — Product Catalog hoàn chỉnh
- **Bao phủ prompt gốc:** `P33,P42`
- **Phụ thuộc:** `T153`
- **File queue:** `tasks/T154_IMPLEMENT_PRODUCT_SEO_SCHEMA.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Triển khai SEO và structured data Product phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Meta fallback/canonical/OG.
2. Product schema.
3. Offer only when valid public price.
4. breadcrumbs.
...
- Hoàn thành đúng trọng tâm: Meta fallback/canonical/OG.
- Hoàn thành đúng trọng tâm: Product schema.
- Hoàn thành đúng trọng tâm: Offer only when valid public price.
- Hoàn thành đúng trọng tâm: breadcrumbs.
...
1. Cập nhật task `T154` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T154` trong message và chỉ stage file thuộc task.
