# T097 — Làm cứng signed context Form blocks
...
- **Phụ thuộc:** `T096`
- **File queue:** `tasks/T097_HARDEN_SIGNED_FORM_CONTEXT.md`
- **Chế độ:** Audit → Implement nếu thiếu/sai → Verify → Test → Commit → Push → Stop

## Kết quả cần đạt

Làm cứng signed context Form blocks phải được đối chiếu với source ở HEAD hiện tại. Giữ nguyên phần đã đúng, sửa phần thiếu/sai bằng thay đổi nhỏ nhất, có test và bằng chứng runtime phù hợp.
...
1. Product/page/block binding, expiry, owner/context.
2. không sửa hidden IDs hoặc replay sai context.
...
- Hoàn thành đúng trọng tâm: Product/page/block binding, expiry, owner/context.
- Hoàn thành đúng trọng tâm: không sửa hidden IDs hoặc replay sai context.
...
1. Cập nhật task `T097` trong `state/STATE.json` thành `DONE`, `VERIFIED`, `FAILED` hoặc `BLOCKED*`.
...
5. Commit có `T097` trong message và chỉ stage file thuộc task.
